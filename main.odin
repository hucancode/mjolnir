...
setup :: proc(engine: ^mjolnir.Engine) {
  ...
  when true {
    ...
    ground_mesh_handle, ground_mesh_ok := create_mesh(engine, geometry.make_quad())
  ...
  when true {
    ...
    for i in 0 ..< LIGHT_COUNT {
      ...
      if should_make_spot_light {
        light_handles[i], light, light_spawn_ok = spawn_spot_light(engine, color, 10, math.PI * 0.25, {6, 2, -1})
        if !light_spawn_ok do continue
        rotate(light, math.PI * 0.4, linalg.VECTOR3F32_X_AXIS)
      } else {
        light_handles[i], light, light_spawn_ok = spawn_point_light(engine, color, 14, {6, 2, -1})
        if !light_spawn_ok do continue
      }
      if plain_material_ok && cube_mesh_ok {
    ...
    if false {
      _, dir_node, ok := spawn_directional_light(engine, {0.2, 0.5, 0.9, 1.0}, true, {0, 10, 0})
      if ok {
  ...
  when true {
    ...
    portal_mesh_handle, portal_mesh_ok := create_mesh(engine, geometry.make_quad())
...
update :: proc(engine: ^mjolnir.Engine, delta_time: f32) {
  ...
  if portal_render_target_index >= 0 {
    portal_rt, rt_ok := get_render_target_camera(engine, portal_render_target_index)
    if rt_ok {
