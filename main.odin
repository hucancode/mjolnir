...
setup :: proc(engine: ^mjolnir.Engine) {
  ...
  when true {
    ...
    // First, log main camera attachments for comparison
    main_cam_ptr := get_main_camera(engine)
    if main_cam_ptr != nil {
      main_final_0 := resources.camera_get_attachment(main_cam_ptr, resources.AttachmentType.FINAL_IMAGE, 0)
      main_final_1 := resources.camera_get_attachment(main_cam_ptr, resources.AttachmentType.FINAL_IMAGE, 1)
      log.infof("MAIN CAMERA - FINAL_IMAGE frame 0: handle=%v index=%d", main_final_0, main_final_0.index)
      log.infof("MAIN CAMERA - FINAL_IMAGE frame 1: handle=%v index=%d", main_final_1, main_final_1.index)
    }
    ...
    log.infof("Portal camera created with handle: %v (index=%d)", portal_camera_handle, portal_camera_handle.index)

    // Log portal camera attachments
    portal_cam, portal_cam_ok := get_camera(engine, portal_camera_handle)
    if portal_cam_ok {
      portal_final_0 := resources.camera_get_attachment(portal_cam, resources.AttachmentType.FINAL_IMAGE, 0)
      portal_final_1 := resources.camera_get_attachment(portal_cam, resources.AttachmentType.FINAL_IMAGE, 1)
      log.infof("PORTAL CAMERA - FINAL_IMAGE frame 0: handle=%v index=%d", portal_final_0, portal_final_0.index)
      log.infof("PORTAL CAMERA - FINAL_IMAGE frame 1: handle=%v index=%d", portal_final_1, portal_final_1.index)
    }
    ...
    log.infof("Portal material created with handle: %v (index=%d)", portal_material_handle, portal_material_handle.index)
...
on_custom_render :: proc(engine: ^mjolnir.Engine, command_buffer: vk.CommandBuffer) {
  ...
  // Transition portal camera's FINAL_IMAGE from COLOR_ATTACHMENT_OPTIMAL to SHADER_READ_ONLY_OPTIMAL
  portal_image := resources.get(engine.resource_manager.image_2d_buffers, portal_texture_handle)
  if portal_image != nil {
