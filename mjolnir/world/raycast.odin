...
world_raycast :: proc(
	...
	// Use persistent node_octree directly
	if world.node_octree.root == nil do return {}

	// Intersection function that filters by mesh tag
	intersection_func :: proc(ray: geometry.Ray, entry: NodeEntry, max_t: f32) -> (hit: bool, t: f32) {
		// Skip non-mesh nodes
		...
		t_near, t_far := geometry.ray_aabb_intersection(ray.origin, inv_dir, entry.bounds)
	...
	result := geometry.octree_raycast(&world.node_octree, ray, config.max_dist, intersection_func)
	if result.hit {
		// Verify tag filter match
...
world_raycast_single :: proc(
	...
	result := geometry.octree_raycast_single(&world.node_octree, ray, config.max_dist, intersection_func)
	if result.hit {
		// Verify tag filter match
		if tag_filter != {} && (tag_filter & result.primitive.tags) != tag_filter do return {}
...
// World raycast - finds all mesh nodes hit by ray
// tag_filter: Only test nodes that have ALL specified tags
world_raycast_multi :: proc(
	...
	entry_results := make([dynamic]geometry.RayHit(NodeEntry), context.temp_allocator)
	geometry.octree_raycast_multi(&world.node_octree, ray, config.max_dist, intersection_func, &entry_results)

	// Convert entry results to node ray hits and filter by tags
	for hit in entry_results {
		if hit.hit {
			...
			append(
				results,
				NodeRayHit {
					node_handle = hit.primitive.handle,
					t = hit.t,
					hit = true,
				},
			)
