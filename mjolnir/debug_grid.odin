...
init_debug_grid :: proc(engine: ^Engine, extent: i32 = 100) -> vk.Result {
  ...
  for i in 0 ..= extent * 2 + 1 {
    x := i - extent
    color :=
      origin_color if x == 0 else (major_color if x % 10 == 0 else minor_color)
  ...
  for i in 0 ..= extent * 2 + 1 {
    z := i - extent
    color :=
      origin_color if z == 0 else (major_color if z % 10 == 0 else minor_color)
    append(
      &verts,
      geometry.Vertex {
  ...
  debug_grid.vertex_buffer = create_host_visible_buffer(
    ctx,
    size,
    {.VERTEX_BUFFER},
  ) or_return
