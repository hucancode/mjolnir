...
Transform :: struct {
  ...
  world_matrix:   matrix[4,4]f32,    // Single world matrix - GPU handles double buffering
}
...
// Helper functions for world matrix access
...
// No-op for compatibility (GPU buffers handle double buffering now)
transform_flush_to_render :: proc(t: ^Transform) {
  // GPU buffers handle double buffering, no action needed
}
...
transform_update_world :: proc(
  ...
  // Update single world matrix - GPU buffers handle double buffering
