...
obb_to_aabb_batch4 :: proc "contextless" (obbs: [4]Obb, aabbs: ^[4]Aabb) {
  if simd_mode == .Scalar {
    // Scalar fallback
    aabbs[0] = obb_to_aabb(obbs[0])
    aabbs[1] = obb_to_aabb(obbs[1])
    aabbs[2] = obb_to_aabb(obbs[2])
    aabbs[3] = obb_to_aabb(obbs[3])
    return
  }
...
quaternion_mul_vector3_batch4 :: proc "contextless" (
  ...
  if simd_mode == .Scalar {
    // Scalar fallback
    result[0] = qmv(q[0], v[0])
    result[1] = qmv(q[1], v[1])
    result[2] = qmv(q[2], v[2])
    result[3] = qmv(q[3], v[3])
    return result
  }
