...
test_frustum_aabb_inside :: proc(t: ^testing.T) {
  ...
  frustum := make_frustum(m)
  // AABB fully inside the frustum
  aabb := Aabb{[3]f32{-0.5, -0.5, -0.5}, [3]f32{0.5, 0.5, 0.5}}
  inside := frustum_test_aabb(frustum, aabb)
...
test_frustum_aabb_outside :: proc(t: ^testing.T) {
  ...
  aabb := Aabb{[3]f32{2, -0.5, -0.5}, [3]f32{3, 0.5, 0.5}}
  inside := frustum_test_aabb(frustum, aabb)
...
test_frustum_aabb_intersect :: proc(t: ^testing.T) {
  ...
  aabb := Aabb{[3]f32{0.5, -0.5, -0.5}, [3]f32{1.5, 0.5, 0.5}}
  inside := frustum_test_aabb(frustum, aabb)
...
test_frustum_sphere_inside :: proc(t: ^testing.T) {
  ...
  center := [3]f32{0, 0, 0}
  radius: f32 = 0.5
  inside := frustum_test_sphere(frustum, center, radius)
...
test_frustum_camera_perspective :: proc(t: ^testing.T) {
  ...
  view_proj := proj * view
  frustum := make_frustum(view_proj)
  ...
  inside_front := frustum_test_aabb(frustum, aabb_front)
  ...
  inside_behind := frustum_test_aabb(frustum, aabb_behind)
  ...
  inside_right := frustum_test_aabb(frustum, aabb_right)
  testing.expect(
    t,
    !inside_right,
  ...
  inside_left := frustum_test_aabb(frustum, aabb_left)
  ...
  inside_above := frustum_test_aabb(frustum, aabb_above)
  ...
  inside_below := frustum_test_aabb(frustum, aabb_below)
  ...
  inside_edge := frustum_test_aabb(frustum, aabb_edge)
  ...
  inside_far := frustum_test_aabb(frustum, aabb_far)
  ...
  inside_near := frustum_test_aabb(frustum, aabb_near)
  testing.expect(
    t,
    !inside_near,
    "AABB before near plane should NOT be visible",
  )
...
test_frustum_camera_orthographic :: proc(t: ^testing.T) {
  ...
  aabb_inside := Aabb{[3]f32{-3, -3, -10}, [3]f32{3, 3, -5}}
  inside := frustum_test_aabb(frustum, aabb_inside)
  testing.expect(
    t,
    inside,
    "AABB within orthographic bounds should be visible",
  )
  log.infof("Inside ortho AABB %v: %v", aabb_inside, inside)
  // Test 2: Object outside orthographic bounds (should NOT be visible)
  aabb_outside := Aabb{[3]f32{-10, -10, -10}, [3]f32{-7, -7, -5}}
  outside := frustum_test_aabb(frustum, aabb_outside)
  testing.expect(
    t,
    !outside,
    "AABB outside orthographic bounds should NOT be visible",
  )
...
test_frustum_camera_moved :: proc(t: ^testing.T) {
  ...
  frustum := make_frustum(view_proj)
  log.infof(
    "Moved camera test: camera at %v looking at %v",
    camera_pos,
    camera_target,
  )
  // Test 1: Object at origin (should be visible since camera is looking at it)
  aabb_origin := Aabb{[3]f32{-1, -1, -1}, [3]f32{1, 1, 1}}
  at_origin := frustum_test_aabb(frustum, aabb_origin)
  testing.expect(t, at_origin, "AABB at origin should be visible")
  log.infof("Origin AABB %v: %v", aabb_origin, at_origin)
  // Test 2: Object behind camera (should NOT be visible)
  aabb_behind_moved := Aabb {
    [3]f32{15, 3, 15}, // Behind the moved camera
    [3]f32{17, 7, 17},
  }
  behind_moved := frustum_test_aabb(frustum, aabb_behind_moved)
  testing.expect(
    t,
    !behind_moved,
    "AABB behind moved camera should NOT be visible",
  )
...
test_frustum_sphere_camera :: proc(t: ^testing.T) {
  ...
  inside_front := frustum_test_sphere(
    frustum,
    sphere_center,
    sphere_radius,
  )
  ...
  log.infof(
    "Front sphere at %v with radius %v: %v",
    sphere_center,
    sphere_radius,
    inside_front,
  )
  ...
  inside_behind := frustum_test_sphere(
    frustum,
    sphere_behind,
    sphere_radius,
  )
  ...
  log.infof(
    "Behind sphere at %v with radius %v: %v",
    sphere_behind,
    sphere_radius,
    inside_behind,
  )
  ...
  inside_large := frustum_test_sphere(
    frustum,
    large_sphere_center,
    large_sphere_radius,
  )
  testing.expect(
    t,
    inside_large,
    "Large sphere intersecting frustum should be visible",
  )
  log.infof(
    "Large sphere at %v with radius %v: %v",
    large_sphere_center,
    large_sphere_radius,
    inside_large,
  )
