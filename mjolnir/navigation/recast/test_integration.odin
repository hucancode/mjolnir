...
create_test_config :: proc(cs, ch: f32) -> Config {
  cfg: Config
  cfg.cs = cs
  cfg.ch = ch
...
test_complete_navmesh_generation_simple :: proc(t: ^testing.T) {
  ...
  // Rasterize triangles
  ok := rasterize_triangles(verts, tris, areas, hf, cfg.walkable_climb)
  ...
  testing.expect(t, len(region_sizes) > 0, "Should create at least one region")
  ...
  testing.expect(
    t,
    largest_region_size >= walkable_spans / 2,
  ...
  testing.expect(
    t,
    total_contour_verts >= 4,
    "Should generate contours with at least 4 vertices for rectangular geometry",
  )
  log.infof(
    "Navigation mesh validation: %d regions, %d walkable spans, %d contour vertices, grid %dx%d",
    len(region_sizes),
    walkable_spans,
    total_contour_verts,
    cfg.width,
...
test_navmesh_with_obstacles :: proc(t: ^testing.T) {
  ...
  areas := []u8 {
    RC_WALKABLE_AREA,
    RC_WALKABLE_AREA,
    RC_NULL_AREA, // Side walls are not walkable
    RC_NULL_AREA,
    RC_NULL_AREA,
    RC_NULL_AREA,
    RC_NULL_AREA,
    ...
    RC_NULL_AREA,
    RC_NULL_AREA,
    RC_WALKABLE_AREA, // Top is walkable
    RC_WALKABLE_AREA,
  }
  cfg := create_test_config(0.3, 0.2)
  cfg.bmin, cfg.bmax = calc_bounds(verts)
  ...
  testing.expect(t, ok, "Failed to erode walkable area")
  // Apply median filter
  ok = median_filter_walkable_area(chf)
  ...
  log.infof(
    "Obstacle validation: %d regions, %d ground spans, %d elevated spans",
    len(region_heights),
    ground_level_spans,
    elevated_level_spans,
  )
}
...
test_navmesh_area_marking :: proc(t: ^testing.T) {
  ...
  verts := [][3]f32{{0, 0, 0}, {40, 0, 0}, {40, 0, 40}, {0, 0, 40}}
  tris := []i32{0, 1, 2, 0, 2, 3}
  areas := []u8{RC_WALKABLE_AREA, RC_WALKABLE_AREA}
  // Create config
  ...
  mark_box_area(box_min, box_max, 10, chf)
  // Mark a cylinder area
  cylinder_pos := [3]f32{30, 0, 30}
  mark_cylinder_area(cylinder_pos, 5.0, 10.0, 20, chf)
  // Mark a convex polygon area
  poly_verts := [][3]f32{{20, -1, 5}, {25, -1, 5}, {25, -1, 10}, {20, -1, 10}}
  mark_convex_poly_area(poly_verts, -1, 10, 30, chf)
  ...
  testing.expect(
    t,
    len(marked_areas) >= 3,
    "Should have multiple area types marked",
  )
...
test_navmesh_performance :: proc(t: ^testing.T) {
  ...
  for y in 0 ..< grid_size {
    for x in 0 ..< grid_size {
      ...
      areas[idx / 3] = RC_WALKABLE_AREA
      areas[idx / 3 + 1] = RC_WALKABLE_AREA
      idx += 6
    }
  }
  // Create config
  ...
  cfg.width, cfg.height = calc_grid_size(cfg.bmin, cfg.bmax, cfg.cs)
  log.infof(
    "Performance test: %d vertices, %d triangles, grid %dx%d",
    vert_count,
    tri_count,
    cfg.width,
    cfg.height,
  ...
  testing.expect(t, ok, "Failed to build regions")
  log.infof(
    "Performance test complete: %d spans, %d regions",
    len(chf.spans),
    chf.max_regions,
  )
}
...
test_edge_cases :: proc(t: ^testing.T) {
  ...
  {
    vertices := [][3]f32{{0, 0, 0}, {1, 0, 0}, {0.5, 0, 1}}
    indices := []i32{0, 1, 2}
    areas := []u8{RC_WALKABLE_AREA}
    cfg := Config {
      cs                      = 0.1,
      ch                      = 0.1,
      walkable_slope          = math.PI * 0.25,
    ...
    if ok {
      testing.expect(
        t,
        pmesh.npolys > 0,
        "Single triangle should produce at least one polygon",
      )
    }
    free_poly_mesh(pmesh)
    free_poly_mesh_detail(dmesh)
