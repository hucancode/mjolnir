...
rc_erode_walkable_area :: proc(radius: i32, chf: ^Rc_Compact_Heightfield) -> bool {
    w := chf.width
    h := chf.height
            ...
            for i in c.index..<c.index + u32(c.count) {
                ...
                for dir in 0..<4 {
                    if rc_get_con(s, dir) != RC_NOT_CONNECTED {
                        ...
                        ni := nc_cell.index + u32(rc_get_con(s, dir))
        ...
        for x := w - 1; x >= 0; x -= 1 {
            ...
            for i in c.index..<c.index + u32(c.count) {
                ...
                if rc_get_con(s, 2) != RC_NOT_CONNECTED {
                    ...
                    ai := ac.index + u32(rc_get_con(s, 2))
                    as := &chf.spans[ai]
                    ...
                    if rc_get_con(as, 1) != RC_NOT_CONNECTED {
                        ...
                        aai := aac.index + u32(rc_get_con(as, 1))
                ...
                if rc_get_con(s, 1) != RC_NOT_CONNECTED {
                    ...
                    ac := &chf.cells[ax + ay * int(w)]
                    ai := ac.index + u32(rc_get_con(s, 1))
                    as := &chf.spans[ai]
                    nd = min(dist[ai] + 2, 250)
                    if nd < dist[i] {
                        dist[i] = nd
                    ...
                    if rc_get_con(as, 0) != RC_NOT_CONNECTED {
                        ...
                        aac := &chf.cells[aax + aay * int(w)]
                        aai := aac.index + u32(rc_get_con(as, 0))
                        nd = min(dist[aai] + 3, 250)
                        if nd < dist[i] {
                            dist[i] = nd
                        }
...
rc_build_distance_field :: proc(chf: ^Rc_Compact_Heightfield) -> bool {
    ...
    if src == nil {
        log.errorf("rc_build_distance_field: Out of memory 'src' (%d)", chf.span_count)
        return false
    }
    defer delete(src)
    ...
    if dst == nil {
        log.errorf("rc_build_distance_field: Out of memory 'dst' (%d)", chf.span_count)
        return false
    }
