...
_build_mesh_parallel_impl :: proc(cset: ^Rc_Contour_Set, nvp: i32, pmesh: ^Rc_Poly_Mesh, config: Parallel_Mesh_Config) -> bool {
    ...
    log.infof("Successfully built polygon mesh in parallel: %d vertices, %d polygons", pmesh.nverts, pmesh.npolys)
    return true
}
...
process_contour_chunk :: proc(build_ctx: ^Mesh_Build_Context, work_item: Contour_Work_Item) -> Contour_Work_Result {
    ...
    for i in work_item.contour_start..<(work_item.contour_start + work_item.contour_count) {
        ...
        max_polygons += int(cont.nverts) - 2
    }
    ...
    vert_coords := make([dynamic]u16, 0, max_vertices * 3)
    ...
    for i in work_item.contour_start..<(work_item.contour_start + work_item.contour_count) {
        ...
        cont := &cset.conts[i]
        if cont.nverts < 3 {
            log.debugf("Worker %d: Skipping contour %d with %d vertices", work_item.worker_id, i, cont.nverts)
            continue
        }
        
        log.debugf("Worker %d: Processing contour %d with %d vertices", work_item.worker_id, i, cont.nverts)
        
        // Convert contour vertices to mesh vertices
        resize(&contour_indices, int(cont.nverts))
        ...
        for j in 0..<cont.nverts {
            ...
            vert_idx := add_vertex(x, y, z, &result.vertices, result.vertex_buckets)
            contour_indices[j] = vert_idx
        }
        
        // Build coordinate array for triangulation
        resize(&vert_coords, len(result.vertices) * 3)
        for v_idx in 0..<len(result.vertices) {
            coord_idx := v_idx * 3
            vert_coords[coord_idx+0] = result.vertices[v_idx].x
            vert_coords[coord_idx+1] = result.vertices[v_idx].y
            vert_coords[coord_idx+2] = result.vertices[v_idx].z
        }
...
merge_worker_results :: proc(build_ctx: ^Mesh_Build_Context, pmesh: ^Rc_Poly_Mesh) -> bool {
    ...
    pmesh.nverts = i32(len(final_vertices))
    pmesh.npolys = i32(total_polygons)
    ...
    for i in 0..<len(final_vertices) {
        ...
        vi := i * 3
        pmesh.verts[vi+0] = vertex.x
        pmesh.verts[vi+1] = vertex.y
        pmesh.verts[vi+2] = vertex.z
    }
