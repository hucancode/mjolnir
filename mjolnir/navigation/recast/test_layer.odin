...
test_simple_layers :: proc(t: ^testing.T) {
  ...
  bmin := [3]f32{0, 0, 0}
  bmax := [3]f32{10, 10, 10}
  hf := create_heightfield(20, 20, bmin, bmax, 0.5, 0.5)
  ...
  // Ground level
  ground_verts := [][3]f32{{0, 0, 0}, {10, 0, 0}, {10, 0, 10}, {0, 0, 10}}
  ground_tris := []i32{0, 1, 2, 0, 2, 3}
  ground_areas := []u8{RC_WALKABLE_AREA, RC_WALKABLE_AREA}
  ...
  // Elevated platform
  platform_verts := [][3]f32{{3, 3, 3}, {7, 3, 3}, {7, 3, 7}, {3, 3, 7}}
  platform_tris := []i32{0, 1, 2, 0, 2, 3}
  platform_areas := []u8{RC_WALKABLE_AREA, RC_WALKABLE_AREA}
  ok = rasterize_triangles(
    platform_verts,
    platform_tris,
    platform_areas,
  ...
  lset, layer_ok := build_heightfield_layers(chf, 0, 2)
...
test_region_overflow_handling :: proc(t: ^testing.T) {
  ...
  hf := create_heightfield(200, 200, bmin, bmax, 0.5, 0.5)
  defer free_heightfield(hf)
  testing.expect(t, hf != nil, "Failed to create heightfield")
  ...
  for row in 0 ..< 16 {
    for col in 0 ..< 16 {
      ...
      rasterize_triangles(verts, tris, areas, hf, 1)
    }
  }
  chf := create_compact_heightfield(2, 1, hf)
  defer free_compact_heightfield(chf)
  ...
  build_regions(chf, 0, 1, 1) // Min area = 1 to keep all small regions
...
test_navtest_layers :: proc(t: ^testing.T) {
  ...
  hf := create_heightfield(305, 258, bmin, bmax, 0.3, 0.2)
  defer free_heightfield(hf)
  testing.expect(t, hf != nil, "Failed to create heightfield")
  ...
  for level in 0 ..< 4 {
    ...
    tris := []i32{0, 1, 2, 0, 2, 3}
    areas := []u8{RC_WALKABLE_AREA, RC_WALKABLE_AREA}
    rasterize_triangles(verts, tris, areas, hf, 4)
  }
  chf := create_compact_heightfield(10, 4, hf)
  defer free_compact_heightfield(chf)
  testing.expect(t, chf != nil, "Failed to build compact heightfield")
  ...
  erode_walkable_area(2, chf)
  build_distance_field(chf)
  build_regions(chf, 0, 8, 20)
  // Count regions
  max_region := 0
  ...
  // Build layers
  lset, layer_ok := build_heightfield_layers(chf, 0, 10)
  defer free_heightfield_layer_set(lset)
