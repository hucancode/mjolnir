...
add_span :: proc(
  ...
  for current_span != nil {
    ...
    // Merge area flags
    if abs(i32(new_span.smax) - i32(current_span.smax)) <=
...
divide_poly :: proc(
  in_verts, out_verts1, out_verts2: [][3]f32,
  axis_offset: f32,
  axis: Axis,
) -> (
  poly1_vert_count: i32,
  poly2_vert_count: i32,
) {
  ...
  for in_vert_a, in_vert_b := 0, len(in_verts) - 1;
    ...
    same_side :=
      (in_vert_axis_delta[in_vert_a] >= 0) ==
      (in_vert_axis_delta[in_vert_b] >= 0)
    if !same_side {
      s :=
        in_vert_axis_delta[in_vert_b] /
...
rasterize_triangle_with_inverse_cs :: proc(
  v0, v1, v2: [3]f32,
  area_id: u8,
  hf: ^Heightfield,
  ...
  for z := z0; z <= z1; z += 1 {
    ...
    for x := x0; x <= x1; x += 1 {
      ...
      span_min_cell_index := u16(
        clamp(
          i32(math.floor(span_min * inverse_cell_height)),
          0,
          RC_SPAN_MAX_HEIGHT,
        ),
      )
      span_max_cell_index := u16(
        clamp(
          i32(math.ceil(span_max * inverse_cell_height)),
          i32(span_min_cell_index) + 1,
          RC_SPAN_MAX_HEIGHT,
        ),
      )
      add_span(
...
rasterize_triangle :: proc(
  ...
  return rasterize_triangle_with_inverse_cs(
    v0,
    v1,
    v2,
    area_id,
...
rasterize_triangles :: proc(
  ...
  for i := 0; i < len(indices); i += 3 {
    ...
    rasterize_triangle_with_inverse_cs(
      v0,
      v1,
      v2,
      area,
