...
test_api_simple_build :: proc(t: ^testing.T) {
  ...
  pmesh, dmesh, ok := build_navmesh(vertices, indices, areas, config)
  ...
  log.infof(
    "API Simple Build Test: Generated %d polygons, %d vertices",
    pmesh.npolys,
    len(pmesh.verts),
  )
}
...
test_api_quick_build :: proc(t: ^testing.T) {
  ...
  vertices := [][3]f32{{0, 0, 0}, {20, 0, 0}, {20, 0, 20}, {0, 0, 20}}
  indices := []i32{0, 1, 2, 0, 2, 3}
  // Create areas array
  ...
  log.infof(
    "API Quick Build Test: Generated %d polygons with cell size 0.5",
    pmesh.npolys,
  )
}
...
test_api_configuration :: proc(t: ^testing.T) {
  ...
  configs := []struct {
    ...
    cs:   f32,
    ch:   f32,
  }{{"Fast", 0.5, 0.5}, {"Balanced", 0.3, 0.2}, {"High_Quality", 0.1, 0.1}}
  for cfg in configs {
    config := Config {
      cs                       = cfg.cs,
      ch                       = cfg.ch,
    ...
    if pmesh != nil {
      testing.expect(
        t,
        pmesh.npolys > 0,
        "Should generate at least one polygon",
      )
    ...
    free_poly_mesh_detail(dmesh)
    log.infof(
      "Config %s: cs=%.2f, ch=%.2f -> %d polygons",
      cfg.name,
      config.cs,
      config.ch,
      pmesh != nil ? pmesh.npolys : 0,
    )
  }
}
...
test_api_error_handling :: proc(t: ^testing.T) {
  ...
  good_areas := []u8{RC_WALKABLE_AREA, RC_WALKABLE_AREA}
...
test_api_build_with_areas :: proc(t: ^testing.T) {
  ...
  areas := []u8 {
    RC_WALKABLE_AREA, // Regular walkable
    RC_WALKABLE_AREA,
    10, // Custom area type
    10,
  }
  ...
  log.infof(
    "API Build with Areas Test: Generated %d area types",
    len(area_counts),
  )
}
...
test_api_comprehensive_pipeline :: proc(t: ^testing.T) {
  ...
  for y in 0 ..< grid_size {
    for x in 0 ..< grid_size {
      ...
      append(&areas, RC_WALKABLE_AREA, RC_WALKABLE_AREA)
      vert_idx += 4
    }
  }
  ...
  for i in 0 ..< pmesh.npolys {
    ...
    for j in 0 ..< pmesh.nvp {
      if pmesh.polys[poly_base + int(pmesh.nvp) + int(j)] !=
         RC_MESH_NULL_IDX {
        has_neighbors = true
        break
      }
  ...
  if pmesh.npolys > 1 {
    ...
    testing.expect(
      ...
      "Most polygons should be connected",
    )
  }
}
