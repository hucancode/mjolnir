      ...
      if c.count > 0 && c.index < u32(len(chf.spans)) {
        ...
        for i in c.index ..< end_idx {
          if i < u32(len(chf.areas)) {
            ...
            if x == 0 || x == 9 || y == 0 || y == 9 {
              testing.expect_value(t, chf.areas[i], RC_NULL_AREA)
            } else {
              // Inner cells should still be walkable
              testing.expect_value(t, chf.areas[i], RC_WALKABLE_AREA)
            }
          }
        }
      }
...
test_build_distance_field :: proc(t: ^testing.T) {
  ...
  testing.expect(
    t,
    hole_adjacent_dist > 0 && hole_adjacent_dist <= 4,
  ...
  testing.expect(
    t,
    interior_dist >= hole_adjacent_dist,
  ...
  adjacent_positions := [][2]i32 {
    ...
    {3, 6}, // Left side
    {4, 3},
    {4, 6}, // Top and bottom of hole
    {5, 3},
    {5, 6}, // Top and bottom of hole
    {6, 3},
    {6, 4},
    {6, 5},
    {6, 6}, // Right side
  ...
  testing.expect(
    t,
    hole_adjacent_cells >= 8 && hole_adjacent_cells <= 12,
            ...
            if i < u32(len(chf.spans)) {
              ...
              if is_edge {
                testing.expect(
                  t,
                  chf.dist[i] == 0,
                  "True edge cells should have distance 0",
                )
              }
            }
          }
...
test_build_regions_watershed :: proc(t: ^testing.T) {
  ...
  testing.expect(
    t,
    region1 != region2,
    "Platforms should have different regions",
  )
}
...
test_region_merging :: proc(t: ^testing.T) {
  ...
  chf := create_compact_heightfield(2, 4, hf)
  defer free_compact_heightfield(chf)
  testing.expect(t, chf != nil, "Building compact heightfield should succeed")
  ok := build_distance_field(chf)
  testing.expect(t, hf != nil, "Building distance field should succeed")
  // Build regions with small min area and large merge area
  // This should create many small regions that get merged
  ok = build_regions(chf, 0, 2, 50)
  testing.expect(t, hf != nil, "Building regions should succeed")
...
test_border_regions :: proc(t: ^testing.T) {
  hf := create_heightfield(14, 14, {-2, -2, -2}, {12, 12, 12}, 1.0, 0.5)
  defer free_heightfield(hf)
  hf.border_size = 2
  testing.expect(t, hf != nil, "Heightfield creation should succeed")
  ...
  testing.expect(t, ok, "Building distance field should succeed")
  // Build regions
  ok = build_regions(chf, hf.border_size, 8, 20)
      ...
      if c.count > 0 && c.index < u32(len(chf.spans)) {
        ...
        for i in c.index ..< end_idx {
          if i < u32(len(chf.spans)) {
            ...
            if x < hf.border_size ||
               x >= chf.width - hf.border_size ||
               y < hf.border_size ||
               y >= chf.height - hf.border_size {
              testing.expect(
                t,
                (s.reg & RC_BORDER_REG) != 0,
                "Border cells should have border flag",
              )
            }
          }
        }
...
test_watershed_region_connectivity :: proc(t: ^testing.T) {
  ...
  hf := create_heightfield(8, 4, {0, 0, 0}, {8, 3, 4}, 1.0, 0.2)
  defer free_heightfield(hf)
  ...
  testing.expect(
    t,
    platform1_region != 0,
    "Platform 1 should be assigned to a region",
  )
  testing.expect(
    t,
    platform2_region != 0,
    "Platform 2 should be assigned to a region",
  )
  testing.expect(
    t,
    platform1_region != platform2_region,
  ...
  testing.expect(
    t,
    platform1_region == platform1_region_alt,
    "All cells in platform 1 should have the same region",
  )
  testing.expect(
    t,
    platform2_region == platform2_region_alt,
  ...
  testing.expect(
    t,
    gap_region == 0,
    "Gap should not be assigned to any region",
  )
}
...
test_region_generation :: proc(t: ^testing.T) {
  ...
  areas := make([]u8, 10)
  defer delete(areas)
  slice.fill(areas, RC_WALKABLE_AREA)
  cfg := Config {
    cs                = 0.5,
    ch                = 0.2,
  ...
  testing.expect(t, hf != nil, "Heightfield creation should succeed")
  // Mark and rasterize
  mark_walkable_triangles(cfg.walkable_slope, vertices, indices, areas)
  ...
  ok = erode_walkable_area(cfg.walkable_radius, chf)
  testing.expect(t, ok, "Erosion should succeed")
  ok = build_distance_field(chf)
  testing.expect(t, ok, "Distance field creation should succeed")
  // Build regions
  ok = build_regions(chf, 0, cfg.min_region_area, cfg.merge_region_area)
