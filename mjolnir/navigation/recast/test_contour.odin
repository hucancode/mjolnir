...
test_build_contours_with_holes :: proc(t: ^testing.T) {
  ...
  for x in 1 ..= 6 {
    for z in 1 ..= 6 {
      add_span(hf, i32(x), i32(z), 0, 4, RC_WALKABLE_AREA, 1)
    }
  }
  ...
  testing.expect(
    t,
    len(contour_set.conts) > 0,
    "Should have contours with holes",
  )
}

@(test)
test_contour_simplification_accuracy :: proc(t: ^testing.T) {
  hf := create_heightfield(6, 6, {0, 0, 0}, {6, 6, 6}, 1.0, 0.5)
  testing.expect(t, hf != nil, "Failed to create heightfield")
  defer free_heightfield(hf)
  ...
  defer free_compact_heightfield(chf)
  ok := build_distance_field(chf)
  ok = build_regions(chf, 2, 8, 20) // Reduced border size to allow interior spans
  // Test different simplification levels
  simplification_levels := []f32{0.0, 1.0, 2.0}
  for level in simplification_levels {
    contour_set := create_contour_set(chf, level, 1, {.WALL_EDGES})
    defer free_contour_set(contour_set)
    ...
    if len(contour_set.conts) > 0 {
      ...
      testing.expect(
        t,
        vertex_count >= 4,
        "Rectangular contour should have at least 4 vertices",
      )
    }
  }
}
...
test_contour_edge_length_constraints :: proc(t: ^testing.T) {
  ...
  for max_edge_len in max_edge_lengths {
    contour_set := create_contour_set(
      chf,
      1.0,
      max_edge_len,
      {.WALL_EDGES},
    ...
    testing.expect(
      t,
      ok,
      "Failed to build contours with edge length constraint",
    )
  }
}
...
test_contour_tessellation_wall_edges :: proc(t: ^testing.T) {
  ...
  contour_set_wall := create_contour_set(chf, 1.0, 3, {.WALL_EDGES})
  defer free_contour_set(contour_set_wall)
...
test_contour_tessellation_area_edges :: proc(t: ^testing.T) {
  ...
  contour_set_area := create_contour_set(chf, 1.0, 3, {.AREA_EDGES})
  defer free_contour_set(contour_set_area)
...
test_contour_boundary_vertices :: proc(t: ^testing.T) {
  ...
  testing.expect(
    t,
    len(contour_set.conts) > 0,
    "Should generate boundary contours",
  )
  if len(contour_set.conts) > 0 {
    contour := &contour_set.conts[0]
    testing.expect(
      t,
      len(contour.verts) >= 4,
      "Boundary contour should have vertices",
    )
  }
}
...
test_contour_l_shaped_region :: proc(t: ^testing.T) {
  ...
  defer free_compact_heightfield(chf)
  ok := build_distance_field(chf)
  ok = build_regions(chf, 1, 10, 15) // Reduced for L-shaped region (18 spans total)
  contour_set := create_contour_set(chf, 1.0, 2, {.WALL_EDGES})
  defer free_contour_set(contour_set)
  testing.expect(t, ok, "Failed to build L-shaped contours")
  testing.expect(
    t,
    len(contour_set.conts) > 0,
    "Should generate L-shaped contours",
  )
  if len(contour_set.conts) > 0 {
    contour := &contour_set.conts[0]
    vertex_count := len(contour.verts)
    ...
    testing.expect(
      t,
      vertex_count > 4,
      "L-shaped contour should have more than 4 vertices",
    )
  }
}
...
test_contour_generation_empty_input :: proc(t: ^testing.T) {
  ...
  testing.expect(
    t,
    chf != nil,
    "Should build compact heightfield even with no walkable area",
  )
  ok := build_distance_field(chf)
  testing.expect(
    t,
    ok,
    "Should build distance field even with no walkable area",
  )
  ok = build_regions(chf, 1, 10, 10)
...
test_contour_generation_simple :: proc(t: ^testing.T) {
  ...
  chf.cells = make([]Compact_Cell, 25)
  chf.spans = make([]Compact_Span, 25)
  ...
  for y in 0 ..< 5 {
    for x in 0 ..< 5 {
      ...
      if x >= 1 && x <= 3 && y >= 1 && y <= 3 {
        ...
        chf.areas[span_idx] = RC_WALKABLE_AREA
      } else {
        chf.spans[span_idx].reg = 0
        chf.areas[span_idx] = RC_NULL_AREA
      }
      ...
      for dir in 0 ..< 4 {
        ...
        ny := y + int(get_dir_offset_y(dir))
        if nx >= 0 && nx < 5 && ny >= 0 && ny < 5 {
          set_con(&chf.spans[span_idx], dir, 0)
        } else {
          set_con(&chf.spans[span_idx], dir, RC_NOT_CONNECTED)
        }
      }
      span_idx += 1
    }
  ...
  // Build contours
  cset := create_contour_set(chf, 1.0, 10)
  ...
  testing.expect(
    t,
    len(cset.conts) == 1,
    "Should have exactly one contour for the square region",
  )
  if len(cset.conts) > 0 {
    cont := &cset.conts[0]
    testing.expect(
...
test_contour_generation_multiple_regions :: proc(t: ^testing.T) {
  ...
  // Allocate cells and spans
  chf.cells = make([]Compact_Cell, 100)
  chf.spans = make([]Compact_Span, 100)
  ...
  testing.expect(
    t,
    len(cset.conts) == 2,
    "Should have exactly two contours for two regions",
  )
  ...
  for i in 0 ..< len(cset.conts) {
    ...
    if cont.reg == 1 {
      ...
      testing.expect(
        t,
        len(cont.verts) >= 4,
        "Region 1 contour should have at least 4 vertices",
      )
    } else if cont.reg == 2 {
      region2_found = true
      testing.expect(
        t,
        len(cont.verts) >= 4,
        "Region 2 contour should have at least 4 vertices",
      )
    }
  }
  testing.expect(
    t,
    region1_found && region2_found,
    "Both regions should have contours",
  )
}
...
test_triangulation_complex_polygon :: proc(t: ^testing.T) {
  ...
  testing.expect(
    t,
    len(triangles) == 18,
    "Octagon should produce 6 triangles (18 indices)",
  )
...
test_triangulation_edge_cases :: proc(t: ^testing.T) {
  // Test 1: Triangle (minimum polygon)
  {
    ...
    triangles := make([dynamic]i32)
    defer delete(triangles)
    ok := triangulate_polygon(verts, indices, &triangles)
    testing.expect(t, ok, "Triangle triangulation should succeed")
    testing.expect(
      t,
      len(triangles) == 3,
      "Triangle should produce 1 triangle (3 indices)",
    )
  }
  ...
  // Test 3: Self-intersecting polygon (bowtie)
  {
    ...
    if ok {
      testing.expect(
        t,
        len(triangles) > 0,
        "If successful, should produce triangles",
      )
      testing.expect(
        t,
        len(triangles) % 3 == 0,
...
test_contour_simplification :: proc(t: ^testing.T) {
  ...
  simplify_contour(raw_verts[:], &simplified, 1.0, 1.0, 0)
  ...
  if len(simplified) >= len(raw_verts) {
    log.warnf(
      "Simplification did not reduce vertex count: %d -> %d (may be due to algorithm tolerance)",
      len(raw_verts),
      len(simplified),
    )
  }
  testing.expect(
    t,
    len(simplified) >= 4,
    "Should have at least 4 vertices for a square",
  )
...
test_simple_square_mesh :: proc(t: ^testing.T) {
  ...
  testing.expect(
    t,
    pmesh != nil,
    "Mesh building from square contour should succeed",
  )
  testing.expect(
    t,
    len(pmesh.verts) == 4,
    "Square should have exactly 4 vertices",
  )
...
test_simple_l_shape_mesh :: proc(t: ^testing.T) {
  ...
  testing.expect(
    t,
    pmesh != nil,
    "Mesh building from L-shaped contour should succeed",
  )
  testing.expect(
    t,
    len(pmesh.verts) == 6,
    "L-shape should have exactly 6 vertices",
  )
  testing.expect(
    t,
    pmesh.npolys >= 2,
    "L-shape should have at least 2 polygons",
  )
}
...
test_contour_to_mesh_pipeline :: proc(t: ^testing.T) {
  ...
  for i in 0 ..< pmesh.npolys {
    ...
    for j in 0 ..< pmesh.nvp {
      if pmesh.polys[poly_idx + int(j)] != RC_MESH_NULL_IDX {
        total_poly_verts += 1
  ...
  testing.expect(
    t,
    total_poly_verts >= 6,
    "Polygons should use at least 6 vertices total",
  )
  testing.expect(
    t,
    len(unique_verts) >= 6,
    "Should use at least 6 unique vertices",
  )
}
...
test_simplify_contour_distance :: proc(t: ^testing.T) {
  ...
  test_cases := []struct {
    ...
    {5, 0, 0, 0, 10, 0, "Point (5,0) on horizontal segment (0,0)-(10,0)"},
    {
      ...
      10,
      0,
      "Point (15,5) outside horizontal segment (0,0)-(10,0)",
    },
  }
...
test_basic_contour_simplification :: proc(t: ^testing.T) {
  ...
  simplify_contour(raw_verts, &simplified, 0.5, 1.0, 12)
  // Should have removed small deviations
  testing.expect(
    t,
    len(simplified) < len(raw_verts),
    "Simplification should reduce vertex count",
  )
  testing.expect(
    t,
    len(simplified) >= 3,
    "Simplified contour should have at least 3 vertices",
  )
  // Test with no simplification (max_error = 0)
  no_simp := make([dynamic][4]i32)
  defer delete(no_simp)
  simplify_contour(raw_verts, &no_simp, 0.0, 1.0, 12)
...
test_simplify_contour_algorithm :: proc(t: ^testing.T) {
  ...
  for max_error in max_errors {
    ...
    testing.expect(
      t,
      len(simplified) > 0,
      "Simplification should produce vertices",
    )
    testing.expect(
      t,
      len(simplified) <= len(raw_verts),
      "Simplified contour should have fewer or equal vertices",
    )
    testing.expect(
      t,
      len(simplified) >= 4,
    ...
    if max_error > 0.01 {
      testing.expect(
        t,
        len(simplified) <= prev_length,
        "Higher error tolerance should produce fewer/equal vertices",
      )
    }
  ...
  // Test with region connections - region boundaries should be preserved
  {
    ...
    simplified: [dynamic][4]i32
    defer delete(simplified)
    simplify_contour(raw_verts_with_regions, &simplified, 1.0, 1.0, 12)
    // Should preserve region boundary vertices
    testing.expect(
      t,
      len(simplified) > 0,
      "Region contour simplification should produce vertices",
    )
    testing.expect(
      t,
      len(simplified) >= 4,
      "Region contour should have at least 4 vertices",
    )
