...
nav_mesh_renderer_init :: proc(renderer: ^Nav_Mesh_Renderer,
    ...
    if !recast.nav_is_ok(vertex_shader_result) {
        return recast.nav_error_chain(bool, .Algorithm_Failed, "Failed to create vertex shader", vertex_shader_result.error)
    }
    ...
    if !recast.nav_is_ok(fragment_shader_result) {
        nav_mesh_renderer_cleanup_partial(renderer)
        return recast.nav_error_chain(bool, .Algorithm_Failed, "Failed to create fragment shader", fragment_shader_result.error)
    }
    ...
    if !recast.nav_is_ok(debug_vertex_shader_result) {
        nav_mesh_renderer_cleanup_partial(renderer)
        return recast.nav_error_chain(bool, .Algorithm_Failed, "Failed to create debug vertex shader", debug_vertex_shader_result.error)
    }
    ...
    if !recast.nav_is_ok(debug_fragment_shader_result) {
        nav_mesh_renderer_cleanup_partial(renderer)
        return recast.nav_error_chain(bool, .Algorithm_Failed, "Failed to create debug fragment shader", debug_fragment_shader_result.error)
    }
    renderer.debug_fragment_shader = debug_fragment_shader

    // Create pipelines
    create_result := nav_mesh_renderer_create_pipelines(renderer)
    if !recast.nav_is_ok(create_result) {
        nav_mesh_renderer_cleanup_partial(renderer)
    ...
    if !recast.nav_is_ok(vertex_buffer_result) {
        nav_mesh_renderer_cleanup_partial(renderer)
        return recast.nav_error_chain(bool, .Algorithm_Failed, "Failed to create vertex buffer", vertex_buffer_result.error)
    }
    ...
    if !recast.nav_is_ok(index_buffer_result) {
        nav_mesh_renderer_cleanup_partial(renderer)
        return recast.nav_error_chain(bool, .Algorithm_Failed, "Failed to create index buffer", index_buffer_result.error)
    }
...
nav_mesh_renderer_build_from_recast :: proc(renderer: ^Nav_Mesh_Renderer,
                                           ...
                                           detail_mesh: ^recast.Poly_Mesh_Detail) -> recast.Nav_Result(bool) {
    if !renderer.initialized {
        return recast.nav_error(bool, .Algorithm_Failed, "Renderer not initialized")
    }

    if poly_mesh == nil {
        return recast.nav_error(bool, .Invalid_Parameter, "Polygon mesh cannot be nil")
    }
    ...
    if !recast.nav_is_ok(vertex_result) {
        return recast.nav_error_chain(bool, .Algorithm_Failed, "Failed to upload navigation mesh vertex data", vertex_result.error)
    }
    ...
    if !recast.nav_is_ok(index_result) {
        return recast.nav_error_chain(bool, .Algorithm_Failed, "Failed to upload navigation mesh index data", index_result.error)
    }
...
nav_mesh_renderer_render :: proc(renderer: ^Nav_Mesh_Renderer, world_matrix: matrix[4,4]f32) -> recast.Nav_Result(bool) {
    if !renderer.initialized {
        return recast.nav_error(bool, .Algorithm_Failed, "Renderer not initialized")
    }
...
nav_mesh_renderer_create_pipelines :: proc(renderer: ^Nav_Mesh_Renderer) -> recast.Nav_Result(bool) {
    ...
    if !recast.nav_is_ok(main_pipeline_result) {
        return recast.nav_error_chain(bool, .Algorithm_Failed, "Failed to create main navigation mesh pipeline", main_pipeline_result.error)
    }
    ...
    if !recast.nav_is_ok(debug_pipeline_result) {
        return recast.nav_error_chain(bool, .Algorithm_Failed, "Failed to create debug navigation mesh pipeline", debug_pipeline_result.error)
    }
