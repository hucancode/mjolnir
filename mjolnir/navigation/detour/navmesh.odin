...
encode_poly_id :: proc(
  ...
  return recast.Poly_Ref(
    (salt << (nav_mesh.poly_bits + nav_mesh.tile_bits)) |
...
navmesh_create_memory_map :: proc(
  data: []u8,
) -> (
  Navmesh_Memory_Map,
  recast.Status,
  ...
  if memory_map.header.vert_count > 0 {
    ...
    memory_map.vertices = slice.from_ptr(
      cast(^[3]f32)&data[offset],
      int(memory_map.header.vert_count),
    )
    offset = vert_bytes
  }
  if memory_map.header.poly_count > 0 {
    ...
    memory_map.polygons = slice.from_ptr(
      cast(^Poly)&data[offset],
      int(memory_map.header.poly_count),
    )
    offset = poly_bytes
  }
  if memory_map.header.max_link_count > 0 {
    link_bytes :=
      offset + size_of(Link) * int(memory_map.header.max_link_count)
    if link_bytes > len(data) do return {}, {.Invalid_Param}
    memory_map.links = slice.from_ptr(
      cast(^Link)&data[offset],
      int(memory_map.header.max_link_count),
    )
    offset = link_bytes
  }
  if memory_map.header.detail_mesh_count > 0 {
    detail_mesh_bytes :=
      offset + size_of(Poly_Detail) * int(memory_map.header.detail_mesh_count)
    if detail_mesh_bytes > len(data) do return {}, {.Invalid_Param}
    memory_map.detail_meshes = slice.from_ptr(
      cast(^Poly_Detail)&data[offset],
      int(memory_map.header.detail_mesh_count),
    )
    offset = detail_mesh_bytes
  }
  if memory_map.header.detail_vert_count > 0 {
    detail_vert_bytes :=
      offset + size_of([3]f32) * int(memory_map.header.detail_vert_count)
    if detail_vert_bytes > len(data) do return {}, {.Invalid_Param}
    memory_map.detail_vertices = slice.from_ptr(
      cast(^[3]f32)&data[offset],
      int(memory_map.header.detail_vert_count),
    )
    offset = detail_vert_bytes
  }
  if memory_map.header.detail_tri_count > 0 {
    detail_tri_bytes :=
      offset + size_of([4]u8) * int(memory_map.header.detail_tri_count)
    if detail_tri_bytes > len(data) do return {}, {.Invalid_Param}
    memory_map.detail_triangles = slice.from_ptr(
      cast(^[4]u8)&data[offset],
      int(memory_map.header.detail_tri_count),
    )
    offset = detail_tri_bytes
  }
  if memory_map.header.bv_node_count > 0 {
    bv_bytes :=
      offset + size_of(BV_Node) * int(memory_map.header.bv_node_count)
    if bv_bytes > len(data) do return {}, {.Invalid_Param}
    memory_map.bv_nodes = slice.from_ptr(
      cast(^BV_Node)&data[offset],
      int(memory_map.header.bv_node_count),
    )
    offset = bv_bytes
  }
  if memory_map.header.off_mesh_con_count > 0 {
    off_mesh_bytes :=
      offset +
      size_of(Off_Mesh_Connection) * int(memory_map.header.off_mesh_con_count)
    if off_mesh_bytes > len(data) do return {}, {.Invalid_Param}
    memory_map.off_mesh_conns = slice.from_ptr(
      cast(^Off_Mesh_Connection)&data[offset],
      int(memory_map.header.off_mesh_con_count),
    )
...
nav_mesh_destroy :: proc(nav_mesh: ^Nav_Mesh) {
  for i in 0 ..< nav_mesh.max_tiles {
    if nav_mesh.tiles[i].header != nil {
      nav_mesh_remove_tile(
        nav_mesh,
        get_tile_ref(nav_mesh, &nav_mesh.tiles[i]),
      )
    }
  }
...
nav_mesh_add_tile :: proc(
  nav_mesh: ^Nav_Mesh,
  data: []u8,
  flags: i32,
  last_ref: recast.Tile_Ref = recast.INVALID_TILE_REF,
) -> (
  tile_ref: recast.Tile_Ref,
  status: recast.Status,
) {
...
connect_int_links :: proc(nav_mesh: ^Nav_Mesh, tile: ^Mesh_Tile) {
  ...
  for i in 0 ..< int(tile.header.poly_count) {
    ...
    for j in 0 ..< int(poly.vert_count) {
      if poly.neis[j] != 0 {
        neighbor_id :=
          poly.neis[j] & 0x8000 != 0 ? poly.neis[j] & 0x7fff : poly.neis[j] - 1
    ...
    for j in 0 ..< nv {
      ...
      for k in 0 ..< nnei - 1 {
        if geometry.segment_segment_intersect_2d(
          ...
          poly_ref :=
            get_poly_ref_base(nav_mesh, neighbor) | recast.Poly_Ref(i)
          ...
          if link_idx, ok := allocate_link(tile, poly_idx); ok {
            ...
            if rev_link_idx, rev_ok := allocate_link(neighbor, u32(i));
               rev_ok {
...
get_poly_ref_base :: proc(
  ...
  return recast.Poly_Ref(
    (salt << (nav_mesh.tile_bits + nav_mesh.poly_bits)) |
    (tile_index << nav_mesh.poly_bits),
  )
}
...
is_edge_on_tile_border :: proc(
  ...
  case 0:
    // East border (max X)
    return(
      math.abs(va.x - bmax.x) < math.F32_EPSILON &&
      math.abs(vb.x - bmax.x) < math.F32_EPSILON \
    )
  case 1:
    // North border (max Z)
    return(
      math.abs(va.z - bmax.z) < math.F32_EPSILON &&
      math.abs(vb.z - bmax.z) < math.F32_EPSILON \
    )
  case 2:
    // West border (min X)
    return(
      math.abs(va.x - bmin.x) < math.F32_EPSILON &&
      math.abs(vb.x - bmin.x) < math.F32_EPSILON \
    )
  case 3:
    // South border (min Z)
    return(
      math.abs(va.z - bmin.z) < math.F32_EPSILON &&
      math.abs(vb.z - bmin.z) < math.F32_EPSILON \
    )
  }
  return false
}
