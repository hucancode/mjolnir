...
test_integration_multiple_pathfinding_operations :: proc(t: ^testing.T) {
  ...
  query1: Nav_Mesh_Query
  defer nav_mesh_query_destroy(&query1)
  query2: Nav_Mesh_Query
  defer nav_mesh_query_destroy(&query2)
  status1 := nav_mesh_query_init(&query1, nav_mesh1, 128)
  testing.expect(
    t,
    recast.status_succeeded(status1),
    "Query1 initialization should succeed",
  )
  status2 := nav_mesh_query_init(&query2, nav_mesh2, 128)
  testing.expect(
    t,
  ...
  half_extents := [3]f32{1.0, 1.0, 1.0}
  status, start_ref1, start_nearest1 := find_nearest_poly(
    &query1,
    start_pos1,
    half_extents,
    &filter,
  ...
  end_nearest1: [3]f32
  status, end_ref1, end_nearest1 = find_nearest_poly(
    &query1,
    end_pos1,
    half_extents,
    &filter,
  ...
  testing.expect(
    t,
    recast.status_succeeded(status),
    "Should find end polygon for query1",
  )
  // Query 2 - longer path
  ...
  status, start_ref2, start_nearest2 = find_nearest_poly(
    &query2,
    start_pos2,
    half_extents,
    &filter,
  ...
  end_nearest2: [3]f32
  status, end_ref2, end_nearest2 = find_nearest_poly(
    &query2,
    end_pos2,
    half_extents,
    &filter,
  ...
  testing.expect(
    t,
    recast.status_succeeded(status),
    "Should find end polygon for query2",
  )
  ...
  path_status1, path_count1 := find_path(
    &query1,
    start_ref1,
    end_ref1,
    start_nearest1,
  ...
  testing.expect(
    t,
    recast.status_succeeded(path_status1),
    "Pathfinding on query1 should succeed",
  )
  testing.expect(
    t,
    path_count1 > 0,
    "Path1 should contain at least one polygon",
  )
  // Then pathfinding on query2
  path_status2, path_count2 := find_path(
    &query2,
    start_ref2,
    end_ref2,
    start_nearest2,
  ...
  testing.expect(
    t,
    recast.status_succeeded(path_status2),
    "Pathfinding on query2 should succeed",
  )
  testing.expect(
    t,
    path_count2 > 0,
    "Path2 should contain at least one polygon",
  )
...
test_navigation_mesh_creation :: proc(t: ^testing.T) {
  ...
  indices := []i32 {
    0,
    2,
    1, // Reversed winding order
    0,
    3,
    2, // Reversed winding order
  }
  ...
  nav_params := Create_Nav_Mesh_Data_Params {
    poly_mesh          = pmesh,
  ...
  nav_data, create_status := create_nav_mesh_data(&nav_params)
  testing.expect(
    t,
    recast.status_succeeded(create_status),
    "Failed to create navigation mesh data",
  )
  ...
  testing.expect(
    ...
    "Failed to initialize navigation mesh",
  )
  ...
  testing.expect(
    ...
    "Failed to add tile to navigation mesh",
  )
}
...
test_pathfinding :: proc(t: ^testing.T) {
  ...
  indices := []i32 {
    ...
    4,
    6,
    5, // Reversed winding order
    4,
    7,
    6, // Reversed winding order
  }
  ...
  nav_mesh, nav_status := create_nav_mesh(
    &Create_Nav_Mesh_Data_Params {
      poly_mesh          = pmesh,
      poly_mesh_detail   = dmesh,
      walkable_height    = f32(config.walkable_height) * config.ch, // Convert cells to world units
      walkable_radius    = f32(config.walkable_radius) * config.cs, // Convert cells to world units
  ...
  testing.expect(
    t,
    recast.status_succeeded(nav_status),
  ...
  query := new(Nav_Mesh_Query)
  defer {
    nav_mesh_query_destroy(query)
    free(query)
  }
  query_init_status := nav_mesh_query_init(query, nav_mesh, 2048)
  testing.expect(
    t,
    recast.status_succeeded(query_init_status),
    "Failed to initialize navigation query",
  )
  // Setup query filter
  filter: Query_Filter
  query_filter_init(&filter)
  ...
  find_status, start_ref, start_nearest := find_nearest_poly(
    query,
    start_pos,
    half_extents,
    &filter,
  ...
  testing.expect(
    ...
    "Failed to find start polygon",
  )
  testing.expect(
    t,
    start_ref != recast.INVALID_POLY_REF,
    "Start polygon should be valid",
  )
  end_ref: recast.Poly_Ref
  end_nearest: [3]f32
  find_status, end_ref, end_nearest = find_nearest_poly(
    query,
    end_pos,
    half_extents,
    &filter,
  ...
  testing.expect(
    t,
    recast.status_succeeded(find_status),
    "Failed to find end polygon",
  )
  testing.expect(
    t,
    end_ref != recast.INVALID_POLY_REF,
    "End polygon should be valid",
  )
  // Find path
  ...
  straight_path := make([]Straight_Path_Point, 256)
  ...
  straight_status, straight_path_count := find_straight_path(
    ...
    nil,
    256,
    u32(Straight_Path_Options.All_Crossings),
  )
  testing.expect(
    t,
  ...
  testing.expect(
    ...
    "Straight path should have at least one point",
  )
...
test_navigation_edge_cases :: proc(t: ^testing.T) {
  ...
  // Test 2: Invalid query positions
  {
    ...
    vertices := [][3]f32{{-5, 0, -5}, {5, 0, -5}, {5, 0, 5}, {-5, 0, 5}}
    indices := []i32 {
      0,
      2,
      1, // Reversed winding order
    ...
    nav_mesh_query_init(query, nav_mesh, 2048)
    filter: Query_Filter
    query_filter_init(&filter)
    // Test positions far outside the mesh
    far_pos := [3]f32{100, 0, 100}
    half_extents := [3]f32{2, 4, 2}
    status, ref, nearest := find_nearest_poly(
      query,
      far_pos,
      half_extents,
      &filter,
    ...
    testing.expect(
      t,
      recast.status_succeeded(status),
      "Query should succeed even if no polygon found",
    )
    testing.expect(
      t,
      ref == recast.INVALID_POLY_REF,
      "Should return invalid ref for far away position",
    )
  }
}
