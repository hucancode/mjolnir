...
test_navmesh_file_serialization :: proc(t: ^testing.T) {
  ...
  nav_mesh, create_ok := create_navmesh(pmesh, dmesh, 2.0, 0.6, 0.9)
  ...
  save_ok := save_navmesh_to_file(nav_mesh, navmesh_file)
  testing.expect(t, save_ok, "Failed to save navigation mesh to file")
  loaded_nav_mesh, load_ok := load_navmesh_from_file(navmesh_file)
  testing.expect(t, load_ok, "Failed to load navigation mesh from file")
  testing.expect(
    t,
    loaded_nav_mesh != nil,
    "Loaded navigation mesh should not be nil",
  )
  defer {
    nav_mesh_destroy(loaded_nav_mesh)
    free(loaded_nav_mesh)
  }
  testing.expect(
    t,
    nav_mesh.params.max_tiles == loaded_nav_mesh.params.max_tiles,
    "Max tiles should match",
  )
  testing.expect(
    t,
    nav_mesh.params.max_polys == loaded_nav_mesh.params.max_polys,
  ...
  testing.expect(
    t,
    original_tile_count == loaded_tile_count,
...
test_navmesh_data_serialization :: proc(t: ^testing.T) {
  vertices := [][3]f32{{-10, 0, -10}, {10, 0, -10}, {10, 0, 10}, {-10, 0, 10}}
  indices := []i32{0, 1, 2, 0, 2, 3}
  areas := []u8{recast.RC_WALKABLE_AREA, recast.RC_WALKABLE_AREA}
  pmesh, dmesh, build_ok := build_test_mesh(vertices, indices, areas)
  testing.expect(t, build_ok, "Failed to build test mesh")
  ...
  testing.expect(
    ...
    "Failed to create nav mesh data",
  )
  ...
  save_ok := save_navmesh_data_to_file(nav_data, data_file)
  testing.expect(t, save_ok, "Failed to save navigation data to file")
  loaded_data, load_ok := load_navmesh_data_from_file(data_file)
  testing.expect(t, load_ok, "Failed to load navigation data from file")
  testing.expect(
    t,
    len(loaded_data) == len(nav_data),
...
test_bake_and_save_workflow :: proc(t: ^testing.T) {
  ...
  nav_mesh, query, runtime_ok := load_navmesh_for_runtime(baked_file)
  testing.expect(t, runtime_ok, "Failed to load navigation mesh for runtime")
  testing.expect(
    t,
    nav_mesh != nil && query != nil,
    "Navigation mesh and query should not be nil",
  )
  defer {
  ...
  path_count, path_status := find_path_points(
    query,
    start_pos,
    end_pos,
    &filter,
...
build_test_mesh :: proc(
  vertices: [][3]f32,
  indices: []i32,
  areas: []u8,
) -> (
  ^recast.Poly_Mesh,
  ^recast.Poly_Mesh_Detail,
  bool,
) {
  cfg: recast.Config
  ...
  chf := recast.create_compact_heightfield(
    cfg.walkable_height,
    cfg.walkable_climb,
    hf,
  )
  ...
  cset := recast.create_contour_set(
    chf,
  ...
  dmesh := recast.create_poly_mesh_detail(
    pmesh,
    chf,
...
count_valid_tiles :: proc(nav_mesh: ^Nav_Mesh) -> int {
  count := 0
