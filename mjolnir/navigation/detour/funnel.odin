...
find_straight_path :: proc(query: ^Nav_Mesh_Query, start_pos, end_pos: [3]f32,
                          ...
                          max_straight_path: i32, options: u32) -> (status: recast.Status, straight_path_count: i32) {
    ...
    for i := i32(0); i < path_count; i += 1 {
        ...
        if geometry.perpendicular_cross_2d(portal_apex, portal_right, right) <= 0.0 {
            if geometry.vector_equal(portal_apex, portal_right) ||
               geometry.perpendicular_cross_2d(portal_apex, portal_left, right) > 0.0 {
                // Tighten funnel
                portal_right = right
        ...
        if geometry.perpendicular_cross_2d(portal_apex, portal_left, left) >= 0.0 {
            if geometry.vector_equal(portal_apex, portal_left) ||
               geometry.perpendicular_cross_2d(portal_apex, portal_right, left) < 0.0 {
                // Tighten funnel
                portal_left = left
...
move_along_surface :: proc(query: ^Nav_Mesh_Query, start_ref: recast.Poly_Ref, start_pos: [3]f32,
    ...
    (result_pos: [3]f32, visited_count: i32, status: recast.Status) {
    visited_count = 0
