...
local_boundary_init :: proc(boundary: ^Local_Boundary, max_segs: i32) -> recast.Status {
    ...
    boundary.polys = make([dynamic]recast.Poly_Ref, 0, max_segs)
...
local_boundary_update :: proc(boundary: ^Local_Boundary, ref: recast.Poly_Ref, pos: [3]f32,
    ...
    for i in 0..<poly_count {
        ...
        if recast.status_failed(get_status) || tile == nil || poly == nil do continue
        ...
        for j in 0..<poly.vert_count {
            ...
            dist_sqr, _ := geometry.point_segment_distance2_2d(pos, va, vb)
            if dist_sqr > collision_query_range*collision_query_range do continue
...
local_boundary_is_valid :: proc(boundary: ^Local_Boundary, pos: [3]f32, radius: f32) -> bool {
    ...
    for segment in boundary.segments {
        ...
        dist_sqr, _ := geometry.point_segment_distance2_2d(pos, seg_start, seg_end)
...
local_boundary_get_segment_poly :: proc(boundary: ^Local_Boundary, index: i32) -> (recast.Poly_Ref, bool) {
...
local_boundary_project_point :: proc(boundary: ^Local_Boundary, pos: [3]f32, radius: f32) -> [3]f32 {
    ...
    for segment in boundary.segments {
        ...
        dist_sqr, _ := geometry.point_segment_distance2_2d(result, seg_start, seg_end)
