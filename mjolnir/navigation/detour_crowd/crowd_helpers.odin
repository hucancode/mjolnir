...
dt_crowd_find_neighbors :: proc(crowd: ^Dt_Crowd) {
    ...
    for i, &agent in crowd.active_agents {
        ...
        for j in 0..<neighbor_count {
            ...
            // Calculate distance
            dx := neighbor_agent.position[0] - agent.position[0]
            dy := neighbor_agent.position[1] - agent.position[1]
            dz := neighbor_agent.position[2] - agent.position[2]
...
dt_crowd_calculate_steering :: proc(crowd: ^Dt_Crowd, dt: f32) {
    ...
    for &agent in crowd.active_agents {
        ...
        if agent.target_state == .Velocity {
            ...
            agent.desired_speed = math.sqrt(agent.desired_velocity[0]*agent.desired_velocity[0] + 
            ...
            dz := target_pos[2] - agent.position[2]
            ...
            if distance > 0.01 {
                ...
                if .Anticipate_Turns in agent.params.update_flags && agent.corner_count > 1 {
                    ...
                    turn_dx := next_corner[0] - target_pos[0]
                    turn_dz := next_corner[2] - target_pos[2]
                    turn_distance := math.sqrt(turn_dx*turn_dx + turn_dz*turn_dz)
                    
                    if turn_distance > 0.01 {
                        turn_dx /= turn_distance
                        turn_dz /= turn_distance
                        ...
                        if distance < blend_distance {
                            ...
                            agent.desired_velocity[0] = dx * (1.0 - blend_factor) + turn_dx * blend_factor * agent.desired_speed
                            agent.desired_velocity[2] = dz * (1.0 - blend_factor) + turn_dz * blend_factor * agent.desired_speed
                        }
                    }
                }
            }
...
dt_crowd_plan_velocity :: proc(crowd: ^Dt_Crowd, dt: f32, debug_data: ^Dt_Crowd_Agent_Debug_Info) {
    ...
    for i, &agent in crowd.active_agents {
        ...
        // Limit velocity
        speed := math.sqrt(agent.new_velocity[0]*agent.new_velocity[0] + agent.new_velocity[2]*agent.new_velocity[2])
        if speed > agent.params.max_speed {
            scale := agent.params.max_speed / speed
            agent.new_velocity[0] *= scale
            agent.new_velocity[2] *= scale
        }
    }
}
...
dt_crowd_integrate :: proc(crowd: ^Dt_Crowd, dt: f32) {
    ...
    for &agent in crowd.active_agents {
        ...
        dv := [3]f32{
            agent.new_velocity[0] - agent.velocity[0],
            agent.new_velocity[1] - agent.velocity[1],
            agent.new_velocity[2] - agent.velocity[2],
        }
        
        dv_len := math.sqrt(dv[0]*dv[0] + dv[1]*dv[1] + dv[2]*dv[2])
        if dv_len > max_delta_v {
            scale := max_delta_v / dv_len
            dv[0] *= scale
            dv[1] *= scale
            dv[2] *= scale
        }
...
dt_crowd_handle_collisions :: proc(crowd: ^Dt_Crowd) {
    ...
    for i, &agent_a in crowd.active_agents {
        ...
        for j in i+1..<len(crowd.active_agents) {
            ...
            dz := agent_b.position[2] - agent_a.position[2]
