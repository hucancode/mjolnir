...
path_queue_init :: proc(queue: ^Path_Queue, max_path_size: i32, max_search_nodes: i32, 
                          nav_query: ^detour.Nav_Mesh_Query) -> nav_recast.Status {
...
path_queue_request :: proc(queue: ^Path_Queue, start_ref, end_ref: nav_recast.Poly_Ref,
                             start_pos, end_pos: [3]f32, filter: ^detour.Query_Filter) -> (Path_Queue_Ref, nav_recast.Status) {
...
path_queue_update :: proc(queue: ^Path_Queue, max_iter: i32) -> nav_recast.Status {
    if queue == nil {
        return {.Invalid_Param}
    }
...
path_queue_get_request_status :: proc(queue: ^Path_Queue, ref: Path_Queue_Ref) -> nav_recast.Status {
    if queue == nil {
        return {.Invalid_Param}
    }
...
path_queue_cancel_request :: proc(queue: ^Path_Queue, ref: Path_Queue_Ref) -> nav_recast.Status {
    if queue == nil {
        return {.Invalid_Param}
    }
...
path_queue_resize :: proc(queue: ^Path_Queue, new_max_queue: i32) -> nav_recast.Status {
...
path_queue_get_query_info :: proc(queue: ^Path_Queue, ref: Path_Queue_Ref) -> (start_ref: nav_recast.Poly_Ref, end_ref: nav_recast.Poly_Ref, 
...
path_queue_force_complete :: proc(queue: ^Path_Queue, ref: Path_Queue_Ref) -> nav_recast.Status {
    if queue == nil || ref == Path_Queue_Ref(0) {
        return {.Invalid_Param}
    }
