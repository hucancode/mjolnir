...
@(private)
_resource_written_in_pass :: proc(
  pass: ^PassInstance,
  res: ResourceKey,
) -> bool {
...
@(private)
_initial_access_for_resource :: proc(
  g: ^Graph,
  res_key: ResourceKey,
) -> (
  ResourceAccess,
  ...
  desc, ok := _get_resource_descriptor(g, res_key)
  if !ok {
    return {}, false
  }
...
_compute_barriers :: proc(g: ^Graph) -> Result {
  ...
  for pass_id in g.execution_order {
    ...
    for input_res in pass.inputs {
      ...
      } else if initial_access, ok := _initial_access_for_resource(
        g,
        input_res,
      ); ok {
        _append_transition_barrier(
...
@(private)
_infer_imported_resource_layout :: proc(
  ...
  case .TEXTURE_2D, .TEXTURE_CUBE:
    ...
    if is_tex {
      if .STORAGE in tex_format.usage {
        access.stage = {.COMPUTE_SHADER}
        access.access = {.SHADER_READ, .SHADER_WRITE}
        access.layout = .GENERAL
      } else if .COLOR_ATTACHMENT in tex_format.usage {
