...
init :: proc(
	...
	log.info("Initializing bloom effect pipeline...")
	...
	log.info("Bloom effect pipeline initialized successfully")
	return .SUCCESS
}
...
// Add bloom node to render graph
add_node :: proc(
	g: ^rg.Graph,
	renderer: ^Renderer,
	texture_manager: ^gpu.TextureManager,
	...
	pass := rg.add_pass(g, "bloom", pass_execute, ctx_storage)
...
pass_execute :: proc(cmd: vk.CommandBuffer, frame_index: u32, user_data: rawptr) {
	...
	push := PushConstant {
		...
		blur_radius       = ctx.config.blur_radius,
