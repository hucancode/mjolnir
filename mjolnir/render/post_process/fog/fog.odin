...
SHADER_FRAG :: #load("../../../shader/fog/frag.spv")
...
init :: proc(self: ^Renderer, gctx: ^gpu.GPUContext, textures_set_layout: vk.DescriptorSetLayout) -> vk.Result {
	log.info("Initializing fog effect pipeline...")
	...
	log.info("Fog effect pipeline initialized successfully")
	return .SUCCESS
}
...
add_node :: proc(g: ^rg.Graph, renderer: ^Renderer, texture_manager: ^gpu.TextureManager, cam_res: ^camera.CameraResources, input_res: rg.ResourceId, extent: vk.Extent2D, frame_index: u32, config: Config, is_final_output: bool, swapchain_res: rg.ResourceId, swapchain_view: vk.ImageView, ctx_storage: ^PassCtx) -> rg.ResourceId {
	output_res := swapchain_res if is_final_output else rg.add_transient_color_texture(g, "fog_output", .B8G8R8A8_SRGB, extent, {.COLOR_ATTACHMENT, .SAMPLED})
	...
	pass := rg.add_pass(g, "fog", pass_execute, ctx_storage)
...
pass_execute :: proc(cmd: vk.CommandBuffer, frame_index: u32, user_data: rawptr) {
	...
	push := PushConstant{input_image_index = input_texture.index, color = ctx.config.color, density = ctx.config.density, start = ctx.config.start, end = ctx.config.end}
