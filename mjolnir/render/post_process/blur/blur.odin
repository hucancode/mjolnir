...
SHADER_FRAG :: #load("../../../shader/blur/frag.spv")
...
init :: proc(
	...
	log.info("Initializing blur effect pipeline...")
	...
	log.info("Blur effect pipeline initialized successfully")
	return .SUCCESS
}
...
add_node :: proc(
	...
	pass := rg.add_pass(g, "blur", pass_execute, ctx_storage)
...
pass_execute :: proc(cmd: vk.CommandBuffer, frame_index: u32, user_data: rawptr) {
	...
	// Resolve input texture from camera resources
	...
	if ctx.is_swapchain_output {
		...
		if !has_tex {
			log.warnf("Blur: Failed to resolve output texture")
			return
		}
		output_tex := gpu.get_texture_2d(ctx.texture_manager, tex_handle)
		if output_tex == nil {
			log.warnf("Blur: get_texture_2d returned nil")
			return
		}
	...
	push := PushConstant {
		...
		weight_falloff    = ctx.config.weight_falloff,
	}
	vk.CmdPushConstants(
		cmd,
		ctx.renderer.pipeline_layout,
