...
// Shared particle data structure
Particle :: struct {
	position:      [3]f32,
	size:          f32,
	velocity:      [3]f32,
...
simulate :: proc(
	...
	params_ptr := gpu.get(params_buffer)
	counter_ptr := gpu.get(particle_count_buffer)
	...
	vk.CmdCopyBuffer(
		command_buffer,
		compact_particle_buffer.buffer,
		particle_buffer.buffer,
		1,
		&vk.BufferCopy{size = vk.DeviceSize(particle_buffer.bytes_count)},
	)
...
compact :: proc(self: ^System, command_buffer: vk.CommandBuffer) {
	gpu.bind_compute_pipeline(
		command_buffer,
...
init :: proc(
	...
	log.debugf("Initializing particle simulation renderer")
...
setup :: proc(
	...
	self.emitter_descriptor_set = gpu.create_descriptor_set(
		...
		{.STORAGE_BUFFER, gpu.buffer_info(particle_count_buffer)},
		{.UNIFORM_BUFFER, gpu.buffer_info(params_buffer)},
	) or_return
	self.compute_descriptor_set = gpu.create_descriptor_set(
		gctx,
		&self.compute_descriptor_set_layout,
		{type = .UNIFORM_BUFFER, info = gpu.buffer_info(params_buffer)},
		{
			type = .STORAGE_BUFFER,
			info = gpu.buffer_info(compact_particle_buffer),
		},
		{
			type = .STORAGE_BUFFER,
			info = gpu.buffer_info(particle_count_buffer),
		},
	) or_return
	self.compact_descriptor_set = gpu.create_descriptor_set(
		...
		{
			type = .STORAGE_BUFFER,
			info = gpu.buffer_info(draw_command_buffer),
		},
		{
			type = .STORAGE_BUFFER,
			info = gpu.buffer_info(particle_count_buffer),
...
teardown :: proc(self: ^System, gctx: ^gpu.GPUContext) {
...
destroy :: proc(self: ^System, gctx: ^gpu.GPUContext) {
	vk.DestroyPipeline(gctx.device, self.compute_pipeline, nil)
	vk.DestroyPipelineLayout(gctx.device, self.compute_pipeline_layout, nil)
	vk.DestroyDescriptorSetLayout(
		gctx.device,
