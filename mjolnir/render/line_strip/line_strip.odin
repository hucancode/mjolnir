...
init :: proc(
	...
	vert_module := gpu.create_shader_module(gctx.device, SHADER_LINE_STRIP_VERT) or_return
	defer vk.DestroyShaderModule(gctx.device, vert_module, nil)
	frag_module := gpu.create_shader_module(gctx.device, SHADER_LINE_STRIP_FRAG) or_return
	defer vk.DestroyShaderModule(gctx.device, frag_module, nil)

	// Create pipeline (LINE_INPUT_ASSEMBLY + LINE_RASTERIZER for line segments)
...
RESOURCES := [?]rg.ResourceSpec{
  ...
  {name = "line_strip_draw_commands", access = .READ},
  {name = "line_strip_draw_count", access = .READ},
}

execute :: proc(manager: $T, resources: ^rg.PassResources, cmd: vk.CommandBuffer, frame_index: u32)
	where type_of(manager.line_strip_renderer) == Renderer &&
	...
	render(
		...
		cam.draw_commands[d.DrawType.LINE_STRIP][frame_index].buffer,
		cam.draw_count[d.DrawType.LINE_STRIP][frame_index].buffer,
