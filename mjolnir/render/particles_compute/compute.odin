...
setup :: proc(
	...
	self.compute_descriptor_set = gpu.create_descriptor_set(
		...
		{type = .STORAGE_BUFFER, info = gpu.buffer_info(&self.particle_count_buffer)},
	) or_return
...
init :: proc(
	...
	create_emitter_pipeline(gctx, self, emitter_set_layout, node_data_set_layout) or_return
	defer if ret != .SUCCESS {
		vk.DestroyDescriptorSetLayout(
			gctx.device,
			self.emitter_descriptor_set_layout,
	...
	create_compute_pipeline(gctx, self, forcefield_set_layout, node_data_set_layout) or_return
	defer if ret != .SUCCESS {
		vk.DestroyDescriptorSetLayout(
			gctx.device,
			self.compute_descriptor_set_layout,
