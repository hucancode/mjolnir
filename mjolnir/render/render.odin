...
// Each PassDecl pairs a static RESOURCES slice with an execute callback.
// At runtime the manager is passed as rawptr (ctx) and cast back to ^Manager.
build_pass_declarations :: proc() -> [dynamic]rg.PassDecl {
  decls := make([dynamic]rg.PassDecl, 0, 22)
  // Compute passes
  append(&decls, rg.PassDecl{name = "particles_compute", scope = .GLOBAL, queue = .COMPUTE, resources = particles_compute.RESOURCES[:], execute = proc(ctx: rawptr, resources: ^rg.PassResources, cmd: vk.CommandBuffer, fi: u32) {particles_compute.execute(cast(^Manager)ctx, resources, cmd, fi)}})
  append(&decls, rg.PassDecl{name = "depth_pyramid", scope = .PER_CAMERA, queue = .COMPUTE, resources = dp.RESOURCES[:], execute = proc(ctx: rawptr, resources: ^rg.PassResources, cmd: vk.CommandBuffer, fi: u32) {dp.execute(cast(^Manager)ctx, resources, cmd, fi)}})
  append(&decls, rg.PassDecl{name = "occlusion_culling", scope = .PER_CAMERA, queue = .COMPUTE, resources = occlusion_culling.RESOURCES[:], execute = proc(ctx: rawptr, resources: ^rg.PassResources, cmd: vk.CommandBuffer, fi: u32) {occlusion_culling.execute(cast(^Manager)ctx, resources, cmd, fi)}})
  append(&decls, rg.PassDecl{name = "shadow_culling_spot", scope = .PER_SPOT_LIGHT, queue = .COMPUTE, resources = shadow_culling_system.RESOURCES[:], execute = proc(ctx: rawptr, resources: ^rg.PassResources, cmd: vk.CommandBuffer, fi: u32) {shadow_culling_system.execute_spot(cast(^Manager)ctx, resources, cmd, fi)}})
  append(&decls, rg.PassDecl{name = "shadow_culling_directional", scope = .PER_DIRECTIONAL_LIGHT, queue = .COMPUTE, resources = shadow_culling_system.RESOURCES[:], execute = proc(ctx: rawptr, resources: ^rg.PassResources, cmd: vk.CommandBuffer, fi: u32) {shadow_culling_system.execute_directional(cast(^Manager)ctx, resources, cmd, fi)}})
  append(&decls, rg.PassDecl{name = "shadow_culling_sphere", scope = .PER_POINT_LIGHT, queue = .COMPUTE, resources = shadow_sphere_culling_system.RESOURCES[:], execute = proc(ctx: rawptr, resources: ^rg.PassResources, cmd: vk.CommandBuffer, fi: u32) {shadow_sphere_culling_system.execute_point(cast(^Manager)ctx, resources, cmd, fi)}})
  // Graphics passes
  append(&decls, rg.PassDecl{name = "shadow_render_spot", scope = .PER_SPOT_LIGHT, queue = .GRAPHICS, resources = shadow_render_system.RESOURCES[:], execute = proc(ctx: rawptr, resources: ^rg.PassResources, cmd: vk.CommandBuffer, fi: u32) {shadow_render_system.execute_spot(cast(^Manager)ctx, resources, cmd, fi)}})
  append(&decls, rg.PassDecl{name = "shadow_render_directional", scope = .PER_DIRECTIONAL_LIGHT, queue = .GRAPHICS, resources = shadow_render_system.RESOURCES[:], execute = proc(ctx: rawptr, resources: ^rg.PassResources, cmd: vk.CommandBuffer, fi: u32) {shadow_render_system.execute_directional(cast(^Manager)ctx, resources, cmd, fi)}})
  append(&decls, rg.PassDecl{name = "shadow_render_sphere", scope = .PER_POINT_LIGHT, queue = .GRAPHICS, resources = shadow_sphere_render_system.RESOURCES[:], execute = proc(ctx: rawptr, resources: ^rg.PassResources, cmd: vk.CommandBuffer, fi: u32) {shadow_sphere_render_system.execute_point(cast(^Manager)ctx, resources, cmd, fi)}})
  append(&decls, rg.PassDecl{name = "geometry", scope = .PER_CAMERA, queue = .GRAPHICS, resources = geometry.RESOURCES[:], execute = proc(ctx: rawptr, resources: ^rg.PassResources, cmd: vk.CommandBuffer, fi: u32) {geometry.execute(cast(^Manager)ctx, resources, cmd, fi)}})
  append(&decls, rg.PassDecl{name = "ambient", scope = .PER_CAMERA, queue = .GRAPHICS, resources = ambient.RESOURCES[:], execute = proc(ctx: rawptr, resources: ^rg.PassResources, cmd: vk.CommandBuffer, fi: u32) {ambient.execute(cast(^Manager)ctx, resources, cmd, fi)}})
  append(&decls, rg.PassDecl{name = "direct_light_point", scope = .PER_CAMERA, queue = .GRAPHICS, resources = direct_light.RESOURCES[:], execute = proc(ctx: rawptr, resources: ^rg.PassResources, cmd: vk.CommandBuffer, fi: u32) {direct_light.execute_point(cast(^Manager)ctx, resources, cmd, fi)}})
  append(&decls, rg.PassDecl{name = "direct_light_spot", scope = .PER_CAMERA, queue = .GRAPHICS, resources = direct_light.RESOURCES[:], execute = proc(ctx: rawptr, resources: ^rg.PassResources, cmd: vk.CommandBuffer, fi: u32) {direct_light.execute_spot(cast(^Manager)ctx, resources, cmd, fi)}})
  append(&decls, rg.PassDecl{name = "direct_light_directional", scope = .PER_CAMERA, queue = .GRAPHICS, resources = direct_light.RESOURCES[:], execute = proc(ctx: rawptr, resources: ^rg.PassResources, cmd: vk.CommandBuffer, fi: u32) {direct_light.execute_directional(cast(^Manager)ctx, resources, cmd, fi)}})
  append(&decls, rg.PassDecl{name = "particles_render", scope = .PER_CAMERA, queue = .GRAPHICS, resources = particles_render.RESOURCES[:], execute = proc(ctx: rawptr, resources: ^rg.PassResources, cmd: vk.CommandBuffer, fi: u32) {particles_render.execute(cast(^Manager)ctx, resources, cmd, fi)}})
  append(&decls, rg.PassDecl{name = "transparent", scope = .PER_CAMERA, queue = .GRAPHICS, resources = transparent.RESOURCES[:], execute = proc(ctx: rawptr, resources: ^rg.PassResources, cmd: vk.CommandBuffer, fi: u32) {transparent.execute(cast(^Manager)ctx, resources, cmd, fi)}})
  append(&decls, rg.PassDecl{name = "wireframe", scope = .PER_CAMERA, queue = .GRAPHICS, resources = wireframe.RESOURCES[:], execute = proc(ctx: rawptr, resources: ^rg.PassResources, cmd: vk.CommandBuffer, fi: u32) {wireframe.execute(cast(^Manager)ctx, resources, cmd, fi)}})
  append(&decls, rg.PassDecl{name = "random_color", scope = .PER_CAMERA, queue = .GRAPHICS, resources = random_color.RESOURCES[:], execute = proc(ctx: rawptr, resources: ^rg.PassResources, cmd: vk.CommandBuffer, fi: u32) {random_color.execute(cast(^Manager)ctx, resources, cmd, fi)}})
  append(&decls, rg.PassDecl{name = "line_strip", scope = .PER_CAMERA, queue = .GRAPHICS, resources = line_strip.RESOURCES[:], execute = proc(ctx: rawptr, resources: ^rg.PassResources, cmd: vk.CommandBuffer, fi: u32) {line_strip.execute(cast(^Manager)ctx, resources, cmd, fi)}})
  append(&decls, rg.PassDecl{name = "post_process", scope = .GLOBAL, queue = .GRAPHICS, resources = post_process.RESOURCES[:], execute = proc(ctx: rawptr, resources: ^rg.PassResources, cmd: vk.CommandBuffer, fi: u32) {post_process.execute(cast(^Manager)ctx, resources, cmd, fi)}})
  append(&decls, rg.PassDecl{name = "ui", scope = .GLOBAL, queue = .GRAPHICS, resources = ui_render.RESOURCES[:], execute = proc(ctx: rawptr, resources: ^rg.PassResources, cmd: vk.CommandBuffer, fi: u32) {ui_render.execute(cast(^Manager)ctx, resources, cmd, fi)}})
  append(&decls, rg.PassDecl{name = "debug_ui", scope = .GLOBAL, queue = .GRAPHICS, resources = debug_ui.RESOURCES[:], execute = proc(ctx: rawptr, resources: ^rg.PassResources, cmd: vk.CommandBuffer, fi: u32) {debug_ui.execute(cast(^Manager)ctx, resources, cmd, fi)}})
  return decls
}
