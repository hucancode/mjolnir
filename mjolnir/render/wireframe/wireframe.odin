...
begin_wireframe_rendering :: proc(
  cmd: vk.CommandBuffer,
...
wireframe_render_pass_deps_from_context :: proc(
  pass_ctx: ^rg.PassContext,
) -> Blackboard {
  return Blackboard {
    final_image = rg.get_texture(pass_ctx, .CAMERA_FINAL_IMAGE),
    depth = rg.get_depth(pass_ctx, .CAMERA_DEPTH),
    draw_commands = rg.get_buffer(pass_ctx, .CAMERA_WIREFRAME_DRAW_COMMANDS),
    draw_count = rg.get_buffer(pass_ctx, .CAMERA_WIREFRAME_DRAW_COUNT),
  }
}

wireframe_render_pass_execute :: proc(
  self: ^Renderer,
  pass_ctx: ^rg.PassContext,
  deps: Blackboard,
) {
  ...
  begin_wireframe_rendering(cmd, final_image_handle, depth_handle)
  render(
    self,
    cmd,
    cam_idx,
