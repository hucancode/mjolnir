...
sprite_write_to_gpu :: proc(
	...
	if handle.index >= MAX_SPRITES {
		log.errorf("Sprite index %d exceeds capacity %d", handle.index, MAX_SPRITES)
		return .ERROR_OUT_OF_DEVICE_MEMORY
	}
	sprite_update_gpu_data(sprite)
	return gpu.write(
		&manager.sprite_buffer,
		&sprite.data,
		int(handle.index),
	)
}
...
create_sprite :: proc(
	...
	sprite_init(sprite, texture, frame_columns, frame_rows, frame_index, color, sampler_index)
