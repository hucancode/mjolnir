...
create_empty_texture_2d :: proc(
	...
	texture^ = gpu.image_create(gpu_context, spec) or_return
...
create_empty_texture_cube :: proc(
	...
	texture.base = gpu.image_create(gpu_context, spec) or_return
	...
	set_texture_cube_descriptor(gpu_context, manager, handle.index, texture.view)
...
create_texture_from_path :: proc(
	...
	spec := gpu.image_spec_2d(u32(width), u32(height), format, usage, generate_mips)

	if generate_mips {
		texture^ = gpu.image_create_with_mipmaps(gpu_context, spec, pixel_data, data_size) or_return
	} else {
		texture^ = gpu.image_create_with_data(gpu_context, spec, pixel_data, data_size) or_return
	}
...
create_texture_from_pixels :: proc(
	...
	spec := gpu.image_spec_2d(u32(width), u32(height), format, {.SAMPLED}, generate_mips)
...
create_texture_from_data :: proc(
	...
	if generate_mips {
		texture^ = gpu.image_create_with_mipmaps(gpu_context, spec, pixels, pixel_size) or_return
	} else {
		texture^ = gpu.image_create_with_data(gpu_context, spec, pixels, pixel_size) or_return
	}
...
create_empty_texture_2d_handle :: proc(
	...
	h, _, ret := create_empty_texture_2d(gpu_context, manager, width, height, format, usage)
...
create_solid_color_texture :: proc(
	...
	return create_texture_from_pixels(gpu_context, manager, pixels, int(width), int(height))
}
...
create_checkerboard_texture :: proc(
	...
	return create_texture_from_pixels(gpu_context, manager, pixels, int(size), int(size))
}
