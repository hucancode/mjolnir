...
load_gltf :: proc(
  ...
  gltf_data := cgltf.parse_file(options, path_cstr) or_return
  defer cgltf.free(gltf_data)

  if len(gltf_data.buffers) > 0 {
    cgltf.load_buffers(options, gltf_data, path_cstr) or_return
  }
  ...
  is_child_node: bit_set[0 ..< 128] // Assuming max 128 nodes, TODO: fix this
...
load_gltf_texture_for_primitive_material :: proc(
  ...
  if pbr_info.base_color_texture.texture == nil {
    ret = .ERROR_UNKNOWN
    return
  }
  g_texture := pbr_info.base_color_texture.texture
  ...
  pixel_data: []u8
  if g_image.uri != nil {
    texture_path_str := path.join({path.dir(root), string(g_image.uri)})
...
process_animations_for_skeletal_mesh :: proc(
  ...
  for &gltf_anim, i in gltf_data.animations {
    ...
    for gltf_channel in gltf_anim.channels {
      ...
      // `.WEIGHTS` not handled for bone animation here
      }
    }
