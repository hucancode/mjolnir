...
ease_elastic_in_out :: proc(x: f32) -> f32 {
	...
	if x == 0 do return 0
	if x == 1 do return 1
	return -(math.pow(2, 20 * x - 10) * math.sin_f32((20 * x - 11.125) * c5)) / 2 if x < 0.5 else (math.pow(2, -20 * x + 10) * math.sin_f32((20 * x - 11.125) * c5)) / 2 + 1
}
...
ease_bounce_in_out :: proc(x: f32) -> f32 {
	return (1 - ease_bounce_out(1 - 2 * x)) / 2 if x < 0.5 else (1 + ease_bounce_out(2 * x - 1)) / 2
}
