...
ui_init :: proc(
    ...
    _, texture := create_texture_from_pixels(engine, mu.default_atlas_alpha[:], mu.DEFAULT_ATLAS_WIDTH, mu.DEFAULT_ATLAS_HEIGHT, 1, .R8_UNORM) or_return
    ui.atlas = texture^
    ...
    data_buffer_write(&ui.proj_buffer, raw_data(&ortho), size_of(linalg.Matrix4f32)) or_return
    ...
    vk.UpdateDescriptorSets(engine.vk_ctx.vkd, len(writes), raw_data(writes[:]), 0, nil)
...
ui_render :: proc(ui: ^UIRenderer, cmd_buf: vk.CommandBuffer) {
	...
	for variant in mu.next_command_iterator(&ui.ctx, &command_backing) {
		...
		case ^mu.Command_Text: ui_draw_text(ui, cmd_buf, cmd.str, cmd.pos, cmd.color)
		case ^mu.Command_Rect: ui_draw_rect(ui, cmd_buf, cmd.rect, cmd.color)
		case ^mu.Command_Icon: ui_draw_icon(ui, cmd_buf, cmd.id, cmd.rect, cmd.color)
		case ^mu.Command_Clip: ui_set_clip_rect(ui, cmd_buf, cmd.rect)
...
ui_flush :: proc(ui: ^UIRenderer, cmd_buf: vk.CommandBuffer) -> vk.Result {
    ...
    data_buffer_write(&ui.vertex_buffer, raw_data(ui.vertices[:]), vk.DeviceSize(size_of(Vertex2D) * ui.vertex_count)) or_return
    data_buffer_write(&ui.index_buffer, raw_data(ui.indices[:]), vk.DeviceSize(size_of(u32) * ui.index_count)) or_return
    vk.CmdBindPipeline(cmd_buf, .GRAPHICS, ui.material.pipeline)
    descriptor_sets := [?]vk.DescriptorSet{ui.material.projection_descriptor_set, ui.material.texture_descriptor_set}
    vk.CmdBindDescriptorSets(cmd_buf, .GRAPHICS, ui.material.pipeline_layout, 0, 2, raw_data(descriptor_sets[:]), 0, nil)
    ...
    ortho := linalg.matrix_ortho3d(0, f32(ui.frame_width), f32(ui.frame_height), 0, -1, 1)
    vk.CmdPushConstants(cmd_buf, ui.material.pipeline_layout, {.VERTEX}, 0, size_of(linalg.Matrix4f32), &ortho)
    offsets := [?]vk.DeviceSize{0}
    vk.CmdBindVertexBuffers(cmd_buf, 0, 1, &ui.vertex_buffer.buffer, raw_data(offsets[:]))
    vk.CmdBindIndexBuffer(cmd_buf, ui.index_buffer.buffer, 0, .UINT32)
    vk.CmdDrawIndexed(cmd_buf, ui.index_count, 1, 0, 0, 0)
    return .SUCCESS
}

ui_push_quad :: proc(ui: ^UIRenderer, cmd_buf: vk.CommandBuffer, dst, src: mu.Rect, color: mu.Color) {
	if (ui.vertex_count >= UI_MAX_VERTICES || ui.index_count >= UI_MAX_INDICES) {
		ui_flush(ui, cmd_buf)
	}
	x, y, w, h := f32(src.x) / mu.DEFAULT_ATLAS_WIDTH,
                  f32(src.y) / mu.DEFAULT_ATLAS_HEIGHT,
                  f32(src.w) / mu.DEFAULT_ATLAS_WIDTH,
                  f32(src.h) / mu.DEFAULT_ATLAS_HEIGHT
...
ui_draw_rect :: proc(ui: ^UIRenderer, cmd_buf: vk.CommandBuffer, rect: mu.Rect, color: mu.Color) {
	ui_push_quad(ui, cmd_buf, rect, mu.default_atlas[mu.DEFAULT_ATLAS_WHITE], color)
}

ui_draw_text :: proc(ui: ^UIRenderer, cmd_buf: vk.CommandBuffer, text: string, pos: mu.Vec2, color: mu.Color) {
...
ui_draw_icon :: proc(ui: ^UIRenderer, cmd_buf: vk.CommandBuffer, id: mu.Icon, rect: mu.Rect, color: mu.Color) {
...
ui_set_clip_rect :: proc(ui: ^UIRenderer, cmd_buf: vk.CommandBuffer, rect: mu.Rect) {
	ui_flush(ui, cmd_buf)
