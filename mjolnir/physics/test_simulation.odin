...
test_cubes_should_not_sink :: proc(t: ^testing.T) {
  ...
  // Create ground
  ground_handle, _ := create_static_body_box(
    &physics_world,
    {10.0, 0.5, 10.0},
    {0, -0.5, 0},
  ...
  // Create sphere
  sphere_handle, _ := create_static_body_sphere(
    &physics_world,
    SPHERE_RADIUS,
    {0, SPHERE_RADIUS, 0},
    linalg.QUATERNIONF32_IDENTITY,
  ...
  cube_handles: [CUBE_COUNT]DynamicRigidBodyHandle
  idx := 0
  for x in 0 ..< NX {
    for y in 0 ..< NY {
      for z in 0 ..< NZ {
        ...
        cube_handles[idx], _ = create_dynamic_body_box(
          &physics_world,
          {1.0, 1.0, 1.0},
          pos,
          linalg.QUATERNIONF32_IDENTITY,
  ...
  for i in 0 ..< 1000 {
    ...
    if i % 100 == 0 {
      ...
      for handle, i in cube_handles {
        if body, ok := get_dynamic_body(&physics_world, handle); ok {
          fmt.printf(
