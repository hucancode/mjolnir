...
test_octree_insert :: proc(t: ^testing.T) {
  ...
  result := geometry.octree_insert(&octree, item1)
  ...
  result = geometry.octree_insert(&octree, item2)
  testing.expect(
    t,
    result == false,
  ...
  geometry.octree_insert(&octree, item3)
  geometry.octree_insert(&octree, item4)
  geometry.octree_insert(&octree, item5)
...
test_octree_remove :: proc(t: ^testing.T) {
  ...
  initial_stats := geometry.octree_get_stats(&octree)
  testing.expect(
    t,
  ...
  result := geometry.octree_remove(&octree, items[1])
  ...
  result = geometry.octree_remove(&octree, fake_item)
...
test_octree_update :: proc(t: ^testing.T) {
  ...
  old_item := item
  item.pos = {5, 5, 5}
  updated := geometry.octree_update(&octree, old_item, item)
...
octree_single_insert_benchmark :: proc(t: ^testing.T) {
  ...
  setup_proc :: proc(
    ...
    options.input = slice.bytes_from_ptr(
      octree_ptr,
      size_of(^geometry.Octree(TestItem)),
    )
    return nil
  }
  bench_proc :: proc(
...
octree_realistic_remove_benchmark :: proc(t: ^testing.T) {
  ...
  bench_proc :: proc(
    ...
    for i in 0 ..< options.rounds {
      ...
      item_index := i % len(bench_data.items)
      removed := geometry.octree_remove(
        bench_data.octree_ptr,
        bench_data.items[item_index],
      )
...
octree_realistic_query_benchmark :: proc(t: ^testing.T) {
  ...
  bench_proc :: proc(
    ...
    for i in 0 ..< options.rounds {
      ...
      geometry.octree_query_aabb_limited(
        octree_ptr,
        query_bounds,
        &results,
        10,
