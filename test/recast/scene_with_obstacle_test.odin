...
test_scene_with_obstacle :: proc(t: ^testing.T) {
    ...
    config.width, config.height = recast.calc_grid_size(config.bmin, config.bmax, config.cs)
    ...
    recast.filter_low_hanging_walkable_obstacles(int(config.walkable_climb), solid)
    recast.filter_ledge_spans(int(config.walkable_height), int(config.walkable_climb), solid)
    ...
    ok = recast.erode_walkable_area(config.walkable_radius, chf)
    ...
    ok = recast.build_regions(chf, config.border_size,
    ...
    ok = recast.build_contours(chf, config.max_simplification_error,
    ...
    ok = recast.build_poly_mesh(cset, config.max_verts_per_poly, pmesh)
    ...
    // Verify polygon mesh
    testing.expect(t, recast.validate_poly_mesh(pmesh), "Polygon mesh should be valid")
    ...
    ok = recast.build_poly_mesh_detail(pmesh, chf,
...
test_contour_with_real_heightfield :: proc(t: ^testing.T) {
    ...
    ok = recast.build_regions(chf, 0, config.min_region_area, config.merge_region_area)
    testing.expect(t, ok, "Failed to build regions")
    ...
    ok = recast.build_contours(chf, config.max_simplification_error, config.max_edge_len, cset)
    testing.expect(t, ok, "Failed to build contours")
