...
test_scene_with_obstacle :: proc(t: ^testing.T) {
    ...
    for i in 0..<cset.nconts {
        cont := &cset.conts[i]
        testing.expect(t, cont.nverts >= 3, "Each contour should have at least 3 vertices")
        total_contour_verts += int(cont.nverts)
    ...
    testing.expect(t, pmesh.nverts > 0, "Should have vertices in polygon mesh")
    testing.expect(t, pmesh.npolys > 0, "Should have polygons in polygon mesh")
    
    log.infof("Built polygon mesh: %d vertices, %d polygons", pmesh.nverts, pmesh.npolys)
    ...
    testing.expect(t, pmesh.nverts >= 4, "Should have at least 4 vertices for ground corners")
    ...
    for i in 0..<pmesh.nverts {
        x := i32(pmesh.verts[i*3 + 0])
        z := i32(pmesh.verts[i*3 + 2])
        min_x = min(min_x, x)
        max_x = max(max_x, x)
        min_z = min(min_z, z)
        max_z = max(max_z, z)
    ...
    testing.expect(t, dmesh.nmeshes == pmesh.npolys, "Should have detail mesh for each polygon")
    ...
    log.infof("  - Detail mesh: %d vertices, %d triangles", dmesh.nverts, dmesh.ntris)
...
test_contour_with_real_heightfield :: proc(t: ^testing.T) {
    ...
    for i in 0..<cset.nconts {
        cont := &cset.conts[i]
        testing.expect(t, cont.nverts >= 3, "Contour should have valid shape")
