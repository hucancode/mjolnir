...
test_basic_navigation_mesh_generation :: proc(t: ^testing.T) {
    ...
    // Calculate bounds
    recast.rc_calc_bounds(verts, 6, &cfg.bmin, &cfg.bmax)
    ...
    ok := recast.rc_create_heightfield(hf, cfg.width, cfg.height, 
                                      cfg.bmin, cfg.bmax, cfg.cs, cfg.ch)
    testing.expect(t, ok, "Failed to create heightfield")
    
    // Rasterize triangles
    ok = recast.rc_rasterize_triangles(verts, 6, tris, areas, 2, hf, 1)
...
test_bounds_calculation :: proc(t: ^testing.T) {
    ...
    bmin, bmax: [3]f32
    recast.rc_calc_bounds(verts, 8, &bmin, &bmax)
...
test_grid_size_calculation :: proc(t: ^testing.T) {
    ...
    recast.rc_calc_grid_size(&bmin, &bmax, 1.0, &width, &height)
    testing.expect_value(t, width, 10)
    testing.expect_value(t, height, 20)
    
    // Test with cell size 0.5
    recast.rc_calc_grid_size(&bmin, &bmax, 0.5, &width, &height)
...
test_compact_heightfield_spans :: proc(t: ^testing.T) {
    ...
    bmin := [3]f32{0, 0, 0}
    bmax := [3]f32{1, 1, 1}
    ok := recast.rc_create_heightfield(hf, 1, 1, bmin, bmax, 1.0, 0.1)
    testing.expect(t, ok, "Failed to create heightfield")
    
    // Add a span manually
    ok = recast.rc_add_span(hf, 0, 0, 0, 10, nav_recast.RC_WALKABLE_AREA, 1)
