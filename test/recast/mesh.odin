...
test_mesh_vertex_hash :: proc(t: ^testing.T) {
  ...
  testing.expect(
    t,
    h1 < recast.RC_VERTEX_BUCKET_COUNT,
    "Hash should be within bucket range",
  )
  testing.expect(
    t,
    h2 < recast.RC_VERTEX_BUCKET_COUNT,
    "Hash should be within bucket range",
  )
}
...
test_validate_poly_mesh :: proc(t: ^testing.T) {
  ...
  pmesh.polys[3], pmesh.polys[4], pmesh.polys[5] =
    recast.RC_MESH_NULL_IDX, recast.RC_MESH_NULL_IDX, recast.RC_MESH_NULL_IDX
  testing.expect(
    t,
