...
level_1_teardown :: proc(user_data: rawptr) -> bool {
	...
	if nodes_to_cleanup != nil {
		log.infof("Level 1 scheduling %d nodes for deletion", len(nodes_to_cleanup))
...
level_2_teardown :: proc(user_data: rawptr) -> bool {
	...
	if nodes_to_cleanup != nil {
		log.infof("Level 2 scheduling %d nodes for deletion", len(nodes_to_cleanup))
		for handle in nodes_to_cleanup {
			mjolnir.queue_node_deletion(engine, handle)
		}
		delete(nodes_to_cleanup)
...
populate_level_test_ui :: proc(engine: ^mjolnir.Engine) {
	...
	if mu.window(ctx, "Level Test", {40, 250, 280, 250}, {.NO_CLOSE}) {
		...
		if !is_transitioning {
			current_name := "None"
			if name, ok := mjolnir.get_current_level_name(&engine.level_manager).?; ok {
				current_name = name
			}
			if .SUBMIT in mu.button(ctx, "Load LV1 (Blocking)") {
				log.infof("========== Button pressed: Load Level 1 Blocking (currently: %s) ==========", current_name)
			...
			if .SUBMIT in mu.button(ctx, "Load LV2 (Blocking)") {
				log.infof("========== Button pressed: Load Level 2 Blocking (currently: %s) ==========", current_name)
			...
			if .SUBMIT in mu.button(ctx, "Load LV1 (Async)") {
				log.infof("========== Button pressed: Load Level 1 Async (currently: %s) ==========", current_name)
				level_1_desc := mjolnir.Level_Descriptor {
					name = "Level 1 - Suzanne",
					setup = level_1_setup,
					teardown = level_1_teardown,
			...
			if .SUBMIT in mu.button(ctx, "Load LV2 (Async)") {
				log.infof("========== Button pressed: Load Level 2 Async (currently: %s) ==========", current_name)
				level_2_desc := mjolnir.Level_Descriptor {
					name = "Level 2 - Mjolnir",
					setup = level_2_setup,
					teardown = level_2_teardown,
