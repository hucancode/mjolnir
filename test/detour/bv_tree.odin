...
test_bv_tree_y_remapping :: proc(t: ^testing.T) {
  ...
  bounds := detour.calc_polygon_bounds_fast(
    &pmesh,
    poly_base,
    pmesh.nvp,
    quant_factor,
...
test_bv_tree_e2e :: proc(t: ^testing.T) {
  ...
  detail_meshes_size :=
    size_of(detour.Poly_Detail) * int(header.detail_mesh_count)
  ...
  bv_nodes := slice.from_ptr(
    cast(^detour.BV_Node)raw_data(nav_data[bv_offset:]),
  ...
  find_status, nearest_poly, nearest_point := detour.find_nearest_poly(
    &query,
    test_point,
    extent,
    &filter,
