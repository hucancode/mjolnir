...
test_bv_tree_e2e :: proc(t: ^testing.T) {
    ...
    testing.expectf(t, header.bv_node_count == 2, "Expected 2 BV nodes (one per poly), got %d", header.bv_node_count)
    ...
    bv_offset := header_size + verts_size + polys_size + links_size + detail_meshes_size + detail_verts_size + detail_tris_size
    bv_nodes := slice.from_ptr(cast(^detour.BV_Node)raw_data(nav_data[bv_offset:]), int(header.bv_node_count))
    for node in bv_nodes {
        testing.expectf(t, node.bmax != [3]u16{1, 1, 1}, "BV node has invalid bounds %v", node.bmax)
    }
    ...
    testing.expectf(t, recast.status_succeeded(init_status), "Failed to init nav mesh: %v", init_status)
    tile_ref, add_status := detour.nav_mesh_add_tile(&nav_mesh, nav_data, 0)
    testing.expectf(t, recast.status_succeeded(add_status), "Failed to add tile: %v", add_status)
    ...
    testing.expectf(t, recast.status_succeeded(query_status), "Failed to init query: %v", query_status)
    ...
    testing.expectf(t, recast.status_succeeded(find_status), "Failed to find nearest poly: %v", find_status)
    testing.expectf(t, nearest_poly != recast.INVALID_POLY_REF, "BV tree query failed - couldn't find polygon at test point")
}
