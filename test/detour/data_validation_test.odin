...
test_endianness_detection :: proc(t: ^testing.T) {
    ...
    magic_little := nav_recast.DT_NAVMESH_MAGIC
...
test_header_validation :: proc(t: ^testing.T) {
    ...
    testing.expect(t, nav_recast.status_succeeded(status), "Valid header should pass validation")
    
    // Test invalid magic
    invalid_header := header
    invalid_header.magic = 0x12345678
    status_magic := validate_navmesh_header_quiet(&invalid_header)
    testing.expect(t, nav_recast.status_failed(status_magic), "Invalid magic should fail validation")
    
    // Test version too old
    old_header := header
    old_header.version = 3
    status_old := validate_navmesh_header_quiet(&old_header)
    testing.expect(t, nav_recast.status_failed(status_old), "Old version should fail validation")
    
    // Test version too new
    new_header := header
    new_header.version = 15
    status_new := validate_navmesh_header_quiet(&new_header)
    testing.expect(t, nav_recast.status_failed(status_new), "Too new version should fail validation")
    ...
    testing.expect(t, nav_recast.status_failed(status_negative), "Negative counts should fail validation")
}
...
test_integration_validation_flow :: proc(t: ^testing.T) {
    ...
    if parsed_header != nil {
        testing.expect_value(t, parsed_header.magic, nav_recast.DT_NAVMESH_MAGIC)
        testing.expect_value(t, parsed_header.version, nav_recast.DT_NAVMESH_VERSION)
    }
