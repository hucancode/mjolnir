...
test_integration_node_pool_queue_interaction :: proc(t: ^testing.T) {
    ...
    max_nodes := i32(100)
    nav_detour.node_pool_init(&pool, max_nodes)
    nav_detour.node_queue_init(&queue, &pool, max_nodes)
    ...
    for i in 0..<max_nodes {
        ...
        node := nav_detour.node_pool_create_node(&pool, ref)
        ...
        nav_detour.node_queue_push(&queue, {ref, node.cost, node.total})
    }
    ...
    for !nav_detour.node_queue_empty(&queue) {
        pathfinding_node := nav_detour.node_queue_pop(&queue)
        testing.expect(t, pathfinding_node.ref != nav_recast.INVALID_POLY_REF, "Popped reference should be valid")
        
        node := nav_detour.node_pool_get_node(&pool, pathfinding_node.ref)
