...
test_detour_reference_encoding :: proc(t: ^testing.T) {
  ...
  decoded_salt, decoded_tile, decoded_poly := detour.decode_poly_id(
    &nav_mesh,
    ref,
  )
...
test_detour_sliced_pathfinding_basic :: proc(t: ^testing.T) {
  ...
  start_status, start_ref, start_nearest := detour.find_nearest_poly(
    &query,
    start_pos,
    half_extents,
    &filter,
  ...
  init_status := detour.init_sliced_find_path(
    &query,
    start_ref,
    end_ref,
    start_nearest,
  ...
  defer delete(path)
  finalize_status, path_count := detour.finalize_sliced_find_path(
    &query,
    path,
    64,
  )
...
test_detour_sliced_pathfinding_partial :: proc(t: ^testing.T) {
  ...
  path := make([]recast.Poly_Ref, 64)
  defer delete(path)
  partial_status, path_count := detour.finalize_sliced_find_path_partial(
    &query,
    existing,
    path,
    64,
...
test_detour_end_to_end_pathfinding :: proc(t: ^testing.T) {
  ...
  for test_case, idx in test_cases {
    ...
    defer delete(path)
    status, path_count := detour.find_path(
      &query,
      start_ref,
      end_ref,
      start_nearest,
...
test_detour_spatial_queries :: proc(t: ^testing.T) {
  ...
  nearest_pt := [3]f32{}
  status, nearest_ref, nearest_pt = detour.find_nearest_poly(
    &query,
    center,
    half_extents,
    &filter,
  ...
  defer delete(polys)
  poly_count: i32
  status, poly_count = detour.query_polygons(
    &query,
    center,
    half_extents,
    &filter,
...
test_detour_raycast_basic :: proc(t: ^testing.T) {
  ...
  defer delete(path)
  path_count := i32(0)
  status, hit, path_count = detour.raycast(
    &query,
    start_ref,
    start_nearest,
    end_pos,
...
test_detour_raycast_wall_hit :: proc(t: ^testing.T) {
  ...
  path := make([]recast.Poly_Ref, 32)
  defer delete(path)
  raycast_status, hit_result, path_count := detour.raycast(
    &query,
    start_ref,
    start_nearest,
    end_pos,
...
test_detour_raycast_with_costs :: proc(t: ^testing.T) {
  ...
  raycast_status, hit, path_count := detour.raycast(
    &query,
    start_ref,
    start_nearest,
    end_pos,
...
test_detour_raycast_errors :: proc(t: ^testing.T) {
  ...
  path := make([]recast.Poly_Ref, 32)
  defer delete(path)
  invalid_status, _, _ := detour.raycast(
    &query,
    invalid_ref,
    pos,
    pos,
  ...
  valid_status, valid_ref, valid_pos := detour.find_nearest_poly(
    &query,
    pos,
    [3]f32{1, 1, 1},
    &filter,
  ...
  zero_status, zero_hit, _ := detour.raycast(
    &query,
    valid_ref,
    valid_pos,
    valid_pos,
...
test_detour_move_along_surface_comprehensive :: proc(t: ^testing.T) {
  ...
  result_pos, visited_count, move_status := detour.move_along_surface(
    &query,
    start_ref,
    start_nearest,
    end_pos,
...
test_detour_move_along_surface_errors :: proc(t: ^testing.T) {
  ...
  visited := make([]recast.Poly_Ref, 16)
  defer delete(visited)
  result_pos, visited_count, invalid_status := detour.move_along_surface(
    &query,
    invalid_ref,
    pos,
    pos,
...
test_detour_error_handling_pathfinding :: proc(t: ^testing.T) {
  ...
  path := make([]recast.Poly_Ref, 64)
  defer delete(path)
  invalid_status, _ := detour.find_path(
    &query,
    invalid_ref,
    invalid_ref,
    pos,
  ...
  valid_status, valid_ref, _ := detour.find_nearest_poly(
    &query,
    pos,
    [3]f32{1, 1, 1},
    &filter,
  ...
  zero_path_status, _ := detour.find_path(
    &query,
    valid_ref,
    valid_ref,
    pos,
...
test_detour_error_handling_spatial_queries :: proc(t: ^testing.T) {
  ...
  zero_status, zero_ref, _ := detour.find_nearest_poly(
    &query,
    pos,
    zero_extents,
    &filter,
  ...
  defer delete(polys)
  query_status, poly_count := detour.query_polygons(
    &query,
    pos,
    [3]f32{1, 1, 1},
    &filter,
  ...
  extreme_status, extreme_ref, _ := detour.find_nearest_poly(
    &query,
    extreme_pos,
    [3]f32{1, 1, 1},
    &filter,
...
test_detour_error_handling_filter_edge_cases :: proc(t: ^testing.T) {
  ...
  pos := [3]f32{5.0, 0.0, 5.0}
  restrict_status, restrict_ref, _ := detour.find_nearest_poly(
    &query,
    pos,
    [3]f32{5, 5, 5},
    &restrictive_filter,
  ...
  permissive_status, permissive_ref, _ := detour.find_nearest_poly(
    &query,
    pos,
    [3]f32{5, 5, 5},
    &permissive_filter,
...
test_detour_edge_cases_boundary_conditions :: proc(t: ^testing.T) {
  ...
  for boundary_pos, i in boundary_positions {
    boundary_status, boundary_ref, _ := detour.find_nearest_poly(
      &query,
      boundary_pos,
      [3]f32{0.5, 0.5, 0.5},
      &filter,
  ...
  same_status, same_ref, same_nearest := detour.find_nearest_poly(
    &query,
    center_pos,
    [3]f32{1, 1, 1},
    &filter,
  ...
  path := make([]recast.Poly_Ref, 64)
  defer delete(path)
  same_path_status, same_path_count := detour.find_path(
    &query,
    same_ref,
    same_ref,
    same_nearest,
...
test_detour_dijkstra_circle_search :: proc(t: ^testing.T) {
  ...
  center_pos := [3]f32{5.0, 0.0, 5.0}
  half_extents := [3]f32{1.0, 1.0, 1.0}
  start_status, start_ref, _ := detour.find_nearest_poly(
    &query,
    center_pos,
    half_extents,
    &filter,
  ...
  search_count, search_status := detour.find_polys_around_circle(
    &query,
    start_ref,
    center_pos,
    radius,
...
test_detour_dijkstra_shape_search :: proc(t: ^testing.T) {
  ...
  defer delete(result_cost)
  search_count, search_status := detour.find_polys_around_shape(
    &query,
    start_ref,
    shape_verts,
    &filter,
...
test_detour_dijkstra_path_extraction :: proc(t: ^testing.T) {
  ...
  if search_count > 1 {
    ...
    defer delete(path)
    path_count, path_status := detour.get_path_from_dijkstra_search(
      &query,
      target_ref,
      path,
      16,
...
test_detour_poly_wall_segments :: proc(t: ^testing.T) {
  ...
  center_pos := [3]f32{5.0, 0.0, 5.0}
  half_extents := [3]f32{1.0, 1.0, 1.0}
  poly_status, poly_ref, _ := detour.find_nearest_poly(
    &query,
    center_pos,
    half_extents,
    &filter,
  ...
  seg_count, seg_status := detour.get_poly_wall_segments(
    &query,
    poly_ref,
    &filter,
    segment_verts,
...
test_detour_edge_case_very_small_extents :: proc(t: ^testing.T) {
  ...
  tiny_status, tiny_ref, _ := detour.find_nearest_poly(
    &query,
    center_pos,
    tiny_extents,
    &filter,
...
create_simple_tile_data :: proc() -> []u8 {
  ...
  poly := cast(^detour.Poly)(uintptr(raw_data(data)) +
    uintptr(header_size) +
    uintptr(vertex_size))
  poly.verts[0] = 0
  poly.verts[1] = 1
  ...
  links := cast(^[4]detour.Link)(uintptr(raw_data(data)) +
    uintptr(header_size) +
    uintptr(vertex_size) +
    uintptr(poly_size))
  for i in 0 ..< 4 {
