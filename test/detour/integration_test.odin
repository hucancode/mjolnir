...
test_integration_multiple_pathfinding_operations :: proc(t: ^testing.T) {
  ...
  half_extents := [3]f32{1.0, 1.0, 1.0}
  status, start_ref1, start_nearest1 := detour.find_nearest_poly(
    &query1,
    start_pos1,
    half_extents,
    &filter,
  ...
  path_status1, path_count1 := detour.find_path(
    &query1,
    start_ref1,
    end_ref1,
    start_nearest1,
  ...
  path_status2, path_count2 := detour.find_path(
    &query2,
    start_ref2,
    end_ref2,
    start_nearest2,
...
test_pathfinding :: proc(t: ^testing.T) {
  ...
  nav_mesh, nav_status := detour.create_nav_mesh(
    &detour.Create_Nav_Mesh_Data_Params {
      poly_mesh          = pmesh,
      poly_mesh_detail   = dmesh,
      walkable_height    = f32(config.walkable_height) * config.ch, // Convert cells to world units
      walkable_radius    = f32(config.walkable_radius) * config.cs, // Convert cells to world units
  ...
  find_status, start_ref, start_nearest := detour.find_nearest_poly(
    query,
    start_pos,
    half_extents,
    &filter,
  ...
  end_ref: recast.Poly_Ref
  end_nearest: [3]f32
  find_status, end_ref, end_nearest = detour.find_nearest_poly(
    query,
    end_pos,
    half_extents,
    &filter,
...
test_navigation_edge_cases :: proc(t: ^testing.T) {
  ...
  // Test 2: Invalid query positions
  {
    ...
    half_extents := [3]f32{2, 4, 2}
    status, ref, nearest := detour.find_nearest_poly(
      query,
      far_pos,
      half_extents,
      &filter,
