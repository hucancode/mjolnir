...
test_frustum_aabb_inside :: proc(t: ^testing.T) {
  ...
  aabb := geometry.Aabb{[3]f32{-0.5, -0.5, -0.5}, [3]f32{0.5, 0.5, 0.5}}
...
test_frustum_aabb_outside :: proc(t: ^testing.T) {
  ...
  aabb := geometry.Aabb{[3]f32{2, -0.5, -0.5}, [3]f32{3, 0.5, 0.5}}
  inside := geometry.frustum_test_aabb(frustum, aabb)
...
test_frustum_aabb_intersect :: proc(t: ^testing.T) {
  ...
  aabb := geometry.Aabb{[3]f32{0.5, -0.5, -0.5}, [3]f32{1.5, 0.5, 0.5}}
  inside := geometry.frustum_test_aabb(frustum, aabb)
...
test_frustum_sphere_inside :: proc(t: ^testing.T) {
  ...
  center := [3]f32{0, 0, 0}
  radius: f32 = 0.5
  inside := geometry.frustum_test_sphere(frustum, center, radius)
...
test_frustum_camera_orthographic :: proc(t: ^testing.T) {
  ...
  aabb_inside := geometry.Aabb{[3]f32{-3, -3, -10}, [3]f32{3, 3, -5}}
  ...
  aabb_outside := geometry.Aabb{[3]f32{-10, -10, -10}, [3]f32{-7, -7, -5}}
...
test_frustum_camera_moved :: proc(t: ^testing.T) {
  ...
  aabb_origin := geometry.Aabb{[3]f32{-1, -1, -1}, [3]f32{1, 1, 1}}
...
test_frustum_sphere_camera :: proc(t: ^testing.T) {
  ...
  inside_front := geometry.frustum_test_sphere(
    frustum,
    sphere_center,
    sphere_radius,
  )
  ...
  inside_behind := geometry.frustum_test_sphere(
    frustum,
    sphere_behind,
    sphere_radius,
  )
  ...
  inside_large := geometry.frustum_test_sphere(
    frustum,
    large_sphere_center,
    large_sphere_radius,
  )
